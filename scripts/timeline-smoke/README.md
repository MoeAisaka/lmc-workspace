...
D25 retains 34px rows and 14px regular secondary text, and wraps each parallel group in the same surface color as its surrounding process, with an 8px radius, 1px border, 10px horizontal / 8px vertical padding and 8px between adjacent groups. The browser fixture includes two adjacent Claude/Codex parallel groups and checks their boundaries at desktop and narrow mobile widths in both themes. Short provider descriptions are preferred; fallback labels show only the operation and executable/file basename. Full commands and individual timing remain in details. All turns default to expanded, explicit folding choices survive completion, and live turns show the latest three steps plus any earlier active/exceptional steps. Earlier successful work can be expanded. Failure, approval and missing-completion states remain visible even when folded. Commentary stays in transcript order outside process surfaces and remains visible. Checks cover 320px mobile width, both themes, status-before-icon order and unchanged final-answer/elapsed-footer contrast.
