 ...
 * pglite WASM/data files and prisma migrations alongside. lmc-cli does NOT depend on the
 * lmc-server workspace package — we reach into the sibling directory at build time only.
 *
 * Layout produced:
 *   <out-dir>/
 *     <platform>/
...
const SERVER_DIR = path.resolve(REPO_ROOT, 'packages/lmc-server');
