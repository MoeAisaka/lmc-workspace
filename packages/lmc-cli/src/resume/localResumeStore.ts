...
import {
    ...
    type ReconnectableLmcSession,
} from './resolveLmcSession';
    ...
    try {
        ...
        if (error instanceof AxiosError && error.response?.status === 401) {
            throw new LocalResumeSessionError(
                'LMC session lookup authentication expired. Run `lmc auth login --force` in this environment.',
                'unavailable',
            );
        }
        return null;
...
export async function resolveLocalReconnectableSession(sessionId: string): Promise<ReconnectableLmcSession> {
    ...
    if (records.length === 0) {
        throw new LocalResumeSessionError(
            `Cannot resume LMC session "${sessionId}" on this machine: no local session encryption data found at ${configuration.sessionsFile}. Start a new LMC session on this machine to enable future resumes.`,
            'not_found',
        );
    }
    ...
    try {
        ...
        const message = error instanceof Error ? error.message : `No LMC session found matching "${sessionId}"`;
        throw new LocalResumeSessionError(
