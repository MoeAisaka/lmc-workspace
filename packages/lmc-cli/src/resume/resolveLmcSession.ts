...
export function parseResumableMetadata(sessionId: string, metadata: unknown): Metadata {
    try {
        ...
        throw new Error(`LMC session ${sessionId} is missing resumable metadata.`);
    }
}

export type ResumableLmcSession = {
    id: string;
    active: boolean;
    metadata: Metadata;
};

export type ReconnectableLmcSession = ResumableLmcSession & {
    seq: number;
    metadataVersion: number;
    agentStateVersion: number;
...
export function resolveSessionRecordByPrefix<T extends { id: string }>(records: T[], sessionId: string): T {
    ...
    if (matches.length === 0) {
        throw new Error(`No LMC session found matching "${trimmed}"`);
    }
    if (matches.length > 1) {
        throw new Error(`Ambiguous LMC session "${trimmed}" matches ${matches.length} sessions. Be more specific.`);
    }
    return matches[0];
}
...
function readAgentCredentials() {
    ...
    if (!credentials) {
        throw new Error(
            `Cannot resume historical LMC sessions through legacy account credentials because ${credentialPath} is missing.`,
        );
    }
    return credentials;
}
...
function resolveSessionEncryption(session: RawSession, credentials: LocalHappyAgentCredentials): RecordEncryption {
    if (session.dataEncryptionKey) {
        ...
        if (!sessionKey) {
            throw new Error(`Failed to decrypt data key for LMC session ${session.id}`);
        }
        return {
            key: sessionKey,
            variant: 'dataKey',
...
function decryptSessionMetadata(session: RawSession, credentials: LocalHappyAgentCredentials): Metadata {
    ...
    if (!metadata) {
        throw new Error(`Failed to decrypt metadata for LMC session ${session.id}`);
    }
    ...
    try {
        ...
        if (error instanceof AxiosError) {
            if (error.response?.status === 401) {
                throw new Error('LMC session lookup authentication expired for legacy account credentials.');
            }
            throw new Error(`Failed to load LMC sessions: ${error.message}`);
        }
        throw error;
    }
}

export async function resolveLmcSession(sessionId: string): Promise<ResumableLmcSession> {
...
export async function resolveReconnectableSession(sessionId: string): Promise<ReconnectableLmcSession> {
    const credentials = readAgentCredentials();
    const sessions = await fetchSessions(credentials);
    const matched = resolveSessionRecordByPrefix(sessions, sessionId);
