 ...
 * through LMC CLI. It manages the agent lifecycle, session state, and
 * communication with the LMC server and mobile app.
 */
...
export async function runGemini(opts: {
  ...
  //
  // Fetch Gemini cloud token (from 'lmc connect gemini')
  //
  ...
  try {
    ...
    if (vendorToken?.oauth?.access_token) {
      ...
      logger.debug('[Gemini] Using OAuth token from LMC center');
  ...
  // (assigned later after LMC server setup)
        ...
        if (msg.status === 'running') {
          ...
          if (errorMessage.includes('Authentication required')) {
            errorMessage = `Authentication required.\n` +
              `For Google Workspace accounts, run: lmc gemini project set <project-id>\n` +
              `Or use a different Google account: lmc connect gemini\n` +
          ...
          if (typeof error === 'object' && error !== null) {
            ...
            // Check for authentication error (Google Workspace accounts need project ID)
              ...
              errorMsg = `Authentication required. For Google Workspace accounts, you need to set a Google Cloud Project:\n` +
                         `  lmc gemini project set <your-project-id>\n` +
                         `Or use a different Google account: lmc connect gemini\n` +
                         `Guide: https://goo.gle/gemini-cli-auth-docs#workspace-gca`;
            }
