...
The daemon is a persistent background process that manages LMC sessions, enables remote control from the mobile app, and handles auto-updates when the CLI version changes.

## 1. Daemon Lifecycle

### Starting the Daemon

Command: `lmc daemon start`

Control Flow:
1. `src/index.ts` receives `daemon start` command
2. Spawns detached process via `spawnLmcCLI(['daemon', 'start-sync'], { detached: true })`
...
3. If mismatch detected:
   - Spawns new daemon via `spawnLmcCLI(['daemon', 'start'])`
...
`lmc doctor` uses `ps aux | grep` to find all LMC processes:
