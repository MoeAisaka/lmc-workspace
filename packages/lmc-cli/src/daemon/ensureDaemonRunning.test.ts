...
vi.mock('./controlClient', () => ({
  isDaemonRunningCurrentlyInstalledLmcVersion: mocks.mockIsDaemonRunningCurrentlyInstalledHappyVersion,
...
describe('ensureDaemonRunning', () => {
  ...
  it('returns without spawning when the daemon is already running', async () => {
    ...
    expect(mocks.mockLoggerDebug).toHaveBeenCalledWith(
      'Ensuring the LMC background service is running & matches our version...',
    )
  })
  ...
  it('starts the daemon and waits for readiness when the installed version is not running', async () => {
    ...
    expect(mocks.mockLoggerDebug).toHaveBeenCalledWith('LMC background service is ready')
  })
})
