...
export async function handleServerCommand(args: string[]): Promise<void> {
    ...
    const dataDir = path.join(configuration.lmcHomeDir, 'server-data');
    ...
    if (!artifacts) {
        console.error(chalk.red('Could not locate the self-host server.'));
        ...
        console.error(chalk.gray('    - sibling packages/lmc-server/sources/standalone.ts in the monorepo'));
    ...
    if (artifacts.bundled) {
        ...
        if (!prismaEngine) {
            ...
            if (artifacts.source === 'package') {
                ...
                console.error(chalk.gray(`  Try reinstalling ${SERVER_PACKAGE_NAME}, then run \`lmc server\` again.`));
            } else {
                console.error(chalk.gray('  Expected @prisma/engines to be available near the link-my-cli package.'));
                console.error(chalk.gray('  Try reinstalling link-my-cli, then run `lmc server` again.'));
            }
            process.exit(1);
        }
    ...
    console.log();
    console.log(chalk.green.bold(`✓ LMC server starting at ${serverUrl}`));
    if (staticDir) {
        console.log(chalk.green(`  Open ${serverUrl} in your browser.`));
    }
    if (opts.persistServerUrl) {
        console.log(chalk.gray('  The LMC CLI + daemon will use this server automatically (settings.serverUrl).'));
    }
...
function showHelp() {
    console.log(`
${chalk.bold('lmc server')} - Run the LMC sync server + web app locally (self-host)

${chalk.bold('Usage:')}
  lmc server [--port 3005] [--host 127.0.0.1] [--reset] [--no-persist]
...
${chalk.bold('Notes:')}
  ...
  - Use ${chalk.cyan('--no-persist')} to run without modifying default LMC settings
...
async function ensureSettingsWriteAllowed(opts: ServerOptions, serverUrl: string): Promise<void> {
    ...
    const message =
        `lmc server will write settings.serverUrl and settings.webappUrl to ${serverUrl} ` +
        `in ${configuration.settingsFile}.`;

    if (!process.stdin.isTTY || !process.stderr.isTTY) {
        console.error(chalk.red('Refusing to modify default LMC settings from a non-interactive run.'));
 ...
 *
 * pkgroll bundles into dist/, so __dirname at runtime is .../lmc-cli/dist; tools/ lives
 * at .../lmc-cli/tools. In rare layouts (e.g. running un-built source via tsx from src/
 * commands/), tools/ sits at .../lmc-cli/tools and __dirname is .../lmc-cli/src/commands,
...
/**
 * Resolves the artifacts needed to spawn the self-host server.
 *
 * Order:
 ...
 *   3. Source-mode fallback for monorepo dev: ../lmc-server/sources/standalone.ts via tsx
 */
...
function resolveInstalledServerPackage(): ServerArtifacts | undefined {
    for (const name of SERVER_PACKAGE_NAMES) {
        const artifacts = resolveServerPackageByName(name);
        if (artifacts) return artifacts;
    }
    return undefined;
}

function resolveServerPackageByName(name: string): ServerArtifacts | undefined {
    try {
        const serverPackage = require_(name) as {
...
function findSourceStandalone(): string | undefined {
    const candidates = [
        path.resolve(__dirname, '../../../lmc-server/sources/standalone.ts'),
        path.resolve(__dirname, '../../lmc-server/sources/standalone.ts'),
        path.resolve(process.cwd(), 'packages/lmc-server/sources/standalone.ts'),
        path.resolve(process.cwd(), '../lmc-server/sources/standalone.ts'),
    ];
...
function findWebappDir(): string | undefined {
    ...
    const candidates = [
        path.resolve(__dirname, '../../../lmc-app/dist'),
        path.resolve(__dirname, '../../lmc-app/dist'),
        path.resolve(process.cwd(), 'packages/lmc-app/dist'),
    ];
    for (const c of candidates) {
...
async function spawnAndWait(art: ServerArtifacts, env: NodeJS.ProcessEnv, args: string[]): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
        ...
        child.on('exit', code => {
            if (code === 0) resolve();
            else reject(new Error(`LMC server ${args[0]} exited with code ${code}`));
        });
    });
}
