import { revokeDeviceLogin } from '@/api/lmcLogout';
...
function showAuthHelp(): void {
  console.log(`
${chalk.bold('lmc auth')} - Authentication management

${chalk.bold('Usage:')}
  lmc auth login [--force]    配对 LMC 设备
  lmc auth logout             撤销设备授权（保留历史和设置）
  lmc auth status             Show authentication status
  lmc auth help               Show this help message
...
async function handleAuthLogin(args: string[]): Promise<void> {
  ...
  if (forceAuth) {
    ...
    const previousCredentials = await readCredentials();
    if (previousCredentials) await revokeDeviceLogin(previousCredentials.token);
  ...
  if (!forceAuth) {
    ...
    if (existingCreds && settings?.machineId) {
      ...
      console.log(chalk.gray(`  Use 'lmc auth login --force' to re-authenticate`));
      return;
...
async function handleAuthLogout(): Promise<void> {
  ...
  console.log(chalk.blue('This will log you out of LMC'));
  console.log(chalk.yellow('⚠️  You will need to re-authenticate to use LMC again'));
  ...
  if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
    try {
      await revokeDeviceLogin(credentials.token);
      // Stop daemon if running
      try {
        await stopDaemon();
        console.log(chalk.gray('Stopped daemon'));
      ...
      console.log(chalk.gray('  Run "lmc auth login" to authenticate again'));
    } catch (error) {
...
async function handleAuthStatus(): Promise<void> {
  ...
  if (!credentials) {
    ...
    console.log(chalk.gray('  Run "lmc auth login" to authenticate'));
    return;
  }
  ...
  if (settings?.machineId) {
    ...
    console.log(chalk.gray('  Run "lmc auth login --force" to fix this'));
  }

  // Data location
  console.log(chalk.gray(`\n  Data directory: ${configuration.lmcHomeDir}`));
