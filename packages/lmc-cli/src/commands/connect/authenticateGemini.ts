...
// Upstream shipped a Google OAuth client hardcoded here. A credential committed to a
// public repository is a credential everyone shares — and Google's own push protection
// flags it — so LMC reads its own from the environment instead. Register an OAuth client
// of type "Desktop app" in Google Cloud Console and export both values before running
// `lmc connect gemini`.
const CLIENT_ID = process.env.LMC_GEMINI_CLIENT_ID;
const CLIENT_SECRET = process.env.LMC_GEMINI_CLIENT_SECRET;

function requireOAuthClient(): { clientId: string; clientSecret: string } {
    if (!CLIENT_ID || !CLIENT_SECRET) {
        throw new Error(
            'Gemini sign-in needs your own Google OAuth client. Set LMC_GEMINI_CLIENT_ID and '
            + 'LMC_GEMINI_CLIENT_SECRET (create a "Desktop app" client in Google Cloud Console), '
            + 'or use `lmc agy` instead — the Gemini backend is deprecated.',
        );
    }
...
async function exchangeCodeForTokens(
    ...
    const response = await fetch(TOKEN_URL, {
        ...
        body: new URLSearchParams({
            ...
            client_id: requireOAuthClient().clientId,
            client_secret: requireOAuthClient().clientSecret,
            code: code,
