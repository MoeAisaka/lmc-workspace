...
export async function handleSandboxStatus(): Promise<void> {
    ...
    if (!config) {
        console.log('Sandbox is not configured. Run `lmc sandbox configure`.');
        return;
    }
...
export function handleSandboxHelp(): void {
    console.log(`
${chalk.bold('lmc sandbox')} - Sandbox management

${chalk.bold('Usage:')}
  lmc sandbox configure      Configure sandbox settings interactively
  lmc sandbox status         Show current sandbox configuration
  lmc sandbox disable        Disable sandboxing
  lmc sandbox help           Show this help
`);
}
