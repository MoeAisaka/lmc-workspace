 ...
 * - connect codex: Store OpenAI API key on the LMC center
 * - connect claude: Store Anthropic API key on the LMC center
 * - connect gemini: Store Gemini API key on the LMC center
...
function showConnectHelp(): void {
    console.log(`
${chalk.bold('lmc connect')} - Connect AI vendor API keys to your LMC center

${chalk.bold('Usage:')}
  lmc connect codex        Store your Codex API key in LMC center
  lmc connect claude       Store your Anthropic API key in LMC center
  lmc connect gemini       Store your Gemini API key in LMC center
  lmc connect status       Show connection status for all vendors
  lmc connect help         Show this help message

${chalk.bold('Description:')}
  The connect command allows you to securely store your AI vendor API keys
  in LMC center. This enables you to use these services through LMC
...
${chalk.bold('Notes:')} 
  • You must be authenticated with LMC first (run 'lmc auth login')
  • API keys are encrypted and stored securely in LMC center
...
async function handleConnectVendor(vendor: 'codex' | 'claude' | 'gemini', displayName: string): Promise<void> {
    console.log(chalk.bold(`\n🔌 Connecting ${displayName} to your LMC center\n`));

    // Check if authenticated
    const credentials = await readCredentials();
    if (!credentials) {
        console.log(chalk.yellow('⚠️  Not authenticated with LMC'));
        console.log(chalk.gray('  Please run "lmc auth login" first'));
        process.exit(1);
    }
...
async function handleConnectStatus(): Promise<void> {
    ...
    console.log('');
    console.log(chalk.gray('To connect a vendor, run: lmc connect <vendor>'));
    console.log(chalk.gray('Example: lmc connect gemini'));
    console.log('');
}

/**
 * Update local Gemini credentials file to keep in sync with the LMC center
 * This ensures the Gemini SDK uses the same account as LMC
 */
