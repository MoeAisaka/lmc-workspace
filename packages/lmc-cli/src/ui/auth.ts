...
export async function doAuth(): Promise<Credentials | null> {
    ...
    console.log('请在浏览器登录 LMC 并批准此设备：');
    console.log(webUrl);
    await openBrowser(webUrl);
    return waitForAuthentication(keypair, pollSecret);
}

/**
 * Wait for authentication to complete and return credentials
 */
async function waitForAuthentication(keypair: tweetnacl.BoxKeyPair, pollSecret: string): Promise<Credentials | null> {
