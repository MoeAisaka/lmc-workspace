...
export function getEnvironmentInfo(): Record<string, any> {
    return {
        PWD: process.env.PWD,
        LMC_HOME_DIR: process.env.LMC_HOME_DIR,
        HAPPY_VARIANT: process.env.HAPPY_VARIANT,
        LMC_SERVER_URL: process.env.LMC_SERVER_URL,
        ...
        happyDir: configuration?.lmcHomeDir,
...
/**
 * Slim daemon status output for `lmc daemon status`
 */
export async function runDoctorDaemon(): Promise<void> {
    ...
    console.log(chalk.gray('\nRun `lmc doctor` for full diagnostics.\n'));
}
...
export async function runDoctorCommand(): Promise<void> {
    ...
    try {
        ...
        if (allProcesses.length > 0) {
            console.log(chalk.bold('🔍 All LMC CLI Processes'));
            ...
            if (allProcesses.length > 1) {
                console.log(chalk.bold('\n💡 Process Management'));
                console.log(chalk.gray('To clean up runaway processes: lmc doctor clean'));
            }
        } else {
    ...
    console.log(`LMC_HOME_DIR: ${env.LMC_HOME_DIR ? chalk.green(env.LMC_HOME_DIR) : chalk.gray('not set')}`);
    console.log(`LMC_SERVER_URL: ${env.LMC_SERVER_URL ? chalk.green(env.LMC_SERVER_URL) : chalk.gray('not set')}`);
    ...
    console.log(`Documentation: ${chalk.blue('https://github.com/MoeAisaka/link-my-cli')}`);

    // ── Concise useful info last (visible without scrolling) ──

    // Basic info
    console.log(chalk.bold('\n📋 Basic Information'));
    console.log(`LMC CLI Version: ${chalk.green(packageJson.version)}`);
    console.log(`Platform: ${chalk.green(process.platform)} ${process.arch}`);
    console.log(`Node.js Version: ${chalk.green(process.version)}`);

    // Configuration
    console.log(chalk.bold('\n⚙️  Configuration'));
    console.log(`LMC Home: ${chalk.blue(configuration.lmcHomeDir)}`);
