...
export async function claudeRemote(opts: {
    ...
    claudeArgs?: string[],
    allowedTools: string[],
    /** Capability only; permissionMode remains the effective decision. */
    allowWorkerPermissionControl?: boolean,
    signal?: AbortSignal,
    ...
    if (opts.sessionId && !claudeCheckSession(opts.sessionId, opts.path)) {
        if (process.env.HAPPY_REFRESH_RECEIVE_SEQ !== undefined) throw new Error('Claude 恢复记录不可用，已拒绝创建替代会话');
        startFrom = null;
    }
