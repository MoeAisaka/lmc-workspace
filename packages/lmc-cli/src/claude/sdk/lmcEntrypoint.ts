 ...
 * NOT itself an SDK entrypoint. The result is that every LMC-driven session
 ...
 * filter set. "remote_mobile" matches LMC's role as a remote-mobile control
 * surface for Claude Code, is part of Claude Code's own allowlist, and keeps
 * LMC-touched sessions visible in `claude --resume`.
 *
 * If the caller already exported CLAUDE_CODE_ENTRYPOINT to something specific,
 * we respect that — the user/operator wins.
 */

export const LMC_DEFAULT_ENTRYPOINT = 'remote_mobile' as const

export function resolveLmcEntrypoint(currentValue: string | undefined): string {
    if (currentValue && currentValue.length > 0) {
        return currentValue
    }
    return LMC_DEFAULT_ENTRYPOINT
}
