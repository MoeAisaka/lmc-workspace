import { runtimeRelease, sdkRequire } from '@/runtime/managedRuntime';
/**
 * Query wrapper around official @anthropic-ai/claude-agent-sdk
 * Maps internal QueryOptions to official SDK Options
 */

import { query as bundledSdkQuery, type Options, type Query } from '@anthropic-ai/claude-agent-sdk'
...
import { resolveLmcEntrypoint } from './lmcEntrypoint'
...
export function query(params: { prompt: QueryPrompt; options?: QueryOptions }): Query {
    ...
    env.CLAUDE_CODE_ENTRYPOINT = resolveLmcEntrypoint(process.env.CLAUDE_CODE_ENTRYPOINT)
    ...
    const sdkQuery: typeof bundledSdkQuery = runtimeRelease('claude') ? sdkRequire()('@anthropic-ai/claude-agent-sdk').query : bundledSdkQuery;
    return sdkQuery({
