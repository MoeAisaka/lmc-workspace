...
export async function runClaude(credentials: Credentials, options: StartOptions = {}): Promise<void> {
    ...
    let metadata: Metadata = {
        ...
        // Both names while readers catch up; see createSessionMetadata.
        ...
        permissionModeSource: initialPermissionMode ? 'explicit' : 'ambient',
        ...workerBirthFromEnv(),
    ...
    if (reconnectSessionId) {
        ...
        restoreReconnectMessageCursor(session, process.env.HAPPY_REFRESH_RECEIVE_SEQ);
        session.updateMetadata((meta) => ({
            ...refreshSessionRuntimeMetadata(meta, metadata),
            sessionCapabilities: metadata.sessionCapabilities,
            sessionConfigState: metadata.sessionConfigState,
            sessionConfigUpdatedAt: Date.now(), sessionConfigError: undefined,
            lifecycleState: 'running',
            archivedBy: undefined,
        }));
    }

    // Fork backfill: when this LMC session was just spawned as a fork
    ...
    // historical messages back to the LMC client — so without an
    ...
    // to the existing LMC session, where the server already has every
    // message it needs.
    ...
    if (!reconnectSessionId && forkClaudeSessionId) {
        ...
        // Bind the new LMC session to the forked Claude UUID up front so the
    ...
    // alongside this LMC session. App-sent prompts also land in the
    ...
    // to the running LMC session. SDK-emitted assistant + tool_result
    ...
    // Agent mail: the tools go on the MCP server both engines already use, and
    // the loop only polls for sessions that opted in.
    const agentMail = startAgentMail(session, credentials.token, () => {
    ...
    const handoffPort = createHandoffPort(session, 'Claude Code', 'claude', () => {
        ...
        if (!id) return null;
        // Read at handover rather than at startup: /clear replaces the thread
        // mid-session, and the one being left is the one running now.
        return { engine: 'Claude Code', id, transcriptPath: join(getProjectPath(workingDirectory), `${id}.jsonl`) };
    });
    const happyServer = await startHappyServer(session, agentMail.mail, handoffPort, { cwd: workingDirectory, machineId, meter });
    // Taking a session over: the briefing goes in the same way agent mail does,
    // as a user message, because that is the only door into a running engine.
    const inherited = await consumePendingHandoff(session);
    if (inherited) {
        logger.debug(`[START] Delivering a ${inherited.source} handoff from the previous engine`);
    ...
    const stopWatchingConfiguration = watchSessionConfiguration(session, 'claude', metadata => {
        if (metadata.permissionMode !== undefined) {
            const chosen = normalizeRemotePermissionMode(metadata.permissionMode ?? undefined);
            // An accepted metadata edit is an explicit control action, not an
            // ambient `default` attached to an old app's ordinary message.
            currentPermissionMode = applySandboxPermissionPolicy(chosen, sandboxEnabled);
        }
    ...
    const currentEnhancedMode = (): EnhancedMode => ({
        ...
        // A hub keeps its hands off the code: no editing, no shell, no subagents.
        disallowedTools: hubDisallowedTools(session.getMetadata(), currentDisallowedTools),
    ...
    session.onUserMessage(async (message) => {
        ...
        const acceptedMetadata = session.getMetadata();
        if (!message.meta?.permissionMode && acceptedMetadata?.orchestration?.role === 'worker') {
            currentPermissionMode = applySandboxPermissionPolicy(normalizeRemotePermissionMode(resolveWorkerPermissionMode(acceptedMetadata, 'claude') ?? undefined), sandboxEnabled);
        }
