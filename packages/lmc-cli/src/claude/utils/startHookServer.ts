 ...
 *     │                                         
 *     ├─► generateHookSettingsFile(port) ──► ~/.lmc/agent/tmp/hooks/session-hook-<pid>.json
 ...
 * 
 * ### Triggered By
 * - `lmc` (fresh start) - new session created
 * - `lmc --continue` - continues last session (may fork)
 * - `lmc --resume` - interactive picker, then resume
 * - `lmc --resume <id>` - resume specific session
 * - `/compact` command - compacts and forks session
 * - Double-escape fork - user forks conversation in CLI
 * 
 * ### Why Not Use File Watching?
 * File watching has race conditions when multiple LMC processes run.
 * With hooks, Claude directly tells THIS specific process about its session,
 * ensuring 1:1 mapping between LMC process and Claude session.
 */
