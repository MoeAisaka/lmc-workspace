...
async function startRemoteRunClaudeHarness(opts: {
    ...
    const sessionClient = {
        ...
        updateMetadata: vi.fn((updater: (current: Record<string, unknown>) => Record<string, unknown>) => {
            ...
            events.emit('metadata', metadata);
        }),
        sendClaudeSessionMessage: vi.fn(),
        onUserMessage: vi.fn(),
        onFileEvent: vi.fn(),
        on: vi.fn(events.on.bind(events)),
        off: vi.fn(events.off.bind(events)),
...
describe('runClaude remote JSONL scanner', () => {
    ...
    it('accepts configuration without inventing a model turn, including explicit default after bypass', async () => {
        const harness = await startRemoteRunClaudeHarness();
        harness.sessionClient.updateMetadata(m => ({ ...m, permissionMode: 'bypassPermissions', modelMode: 'claude-opus-5', effortLevel: 'high' }));
        expect(harness.loopOptions.messageQueue.queue).toEqual([]);
        const incoming = harness.sessionClient.onUserMessage.mock.calls[0][0];
        await incoming({ content: { text: 'one real user turn' }, meta: {} });
        expect(harness.loopOptions.messageQueue.queue[0].mode).toMatchObject({ permissionMode: 'bypassPermissions', model: 'claude-opus-5', effort: 'high' });
        harness.sessionClient.updateMetadata(m => ({ ...m, permissionMode: 'default' }));
        expect(harness.loopOptions.messageQueue.queue).toHaveLength(1);
        await incoming({ content: { text: 'next real turn' }, meta: {} });
        expect(harness.loopOptions.messageQueue.queue[1].mode.permissionMode).toBe('default');
        await harness.finish();
    });
