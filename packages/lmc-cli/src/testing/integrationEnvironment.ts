...
export function applyEnvironmentToProcess(env: IntegrationEnvironment) {
    process.env.LMC_SERVER_URL = `http://localhost:${env.serverPort}`;
    process.env.LMC_WEBAPP_URL = `http://localhost:${env.expoPort}`;
    process.env.LMC_HOME_DIR = join(env.envDir, 'cli', 'home');
