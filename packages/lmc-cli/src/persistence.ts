 ...
 * 
 * Handles settings and private key storage in ~/.lmc/agent/ or a local .happy/
 */
...
export interface DaemonLocallyPersistedState {
  ...
  /** Set when the process was started by the OS service manager, so a later
   *  launchd invocation can tell an already-supervised daemon from one a
   *  session or an upgrade left behind. */
  supervisor?: 'launchd';
}
...
export async function writeSettings(settings: Settings): Promise<void> {
  if (!existsSync(configuration.lmcHomeDir)) {
    await mkdir(configuration.lmcHomeDir, { recursive: true })
  }
 ...
 */
/**
 * A heartbeat rewrites the whole state file. Anything startup recorded — the
 * supervisor above all — has to survive it, or the next launchd invocation sees
 * an unsupervised daemon and takes the role over on every tick.
 */
export function heartbeatDaemonState(
  base: DaemonLocallyPersistedState,
  patch: Pick<DaemonLocallyPersistedState, 'pid' | 'httpPort' | 'startedWithCliVersion'>,
  at: string,
): DaemonLocallyPersistedState {
  return { ...base, ...patch, lastHeartbeat: at };
}
...
export async function acquireDaemonLock(
  ...
  lockFile: string = configuration.daemonLockFile
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      return await open(lockFile, constants.O_RDONLY);
    } catch (error: any) {
      try {
        unlinkSync(tempPath);
      } catch { }
 ...
 */
export async function releaseDaemonLock(lockHandle: FileHandle, lockFile: string = configuration.daemonLockFile): Promise<void> {
  try {
