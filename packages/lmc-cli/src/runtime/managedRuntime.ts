...
export function engineCapabilities(engine:Engine) {
    return {modelDiscovery:true,refresh:true,cancelRefresh:true,turnQueue:true,turnQueueLifecycle:true,authentication:true,authenticationRecovery:true,resume:true,resourceFiles:true,resourceSearch:true,fileInbox:true,model:true,effort:true,context:engine==='codex',serviceTier:engine==='codex',runtimeConfiguration:engine==='codex'};
}
