...
import { execSync, execFileSync, type ChildProcess } from 'node:child_process';
...
function readCodexCliVersion(): { major: number; minor: number; patch: number } | null {
    try {
        const version = execFileSync(codexExecutable(), ['--version'], { encoding: 'utf8', windowsHide: true }).trim();
...
export class CodexAppServerClient {
    ...
    private emitRawTurnCompletion(
        ...
        // A stale completion must not clear the active turn or emit an idle event,
        // even when the promise resolver itself would reject that completion.
        const activeTurnId = this.pendingTurnCompletion?.turnId ?? this._turnId;
        if ((turnId && activeTurnId && turnId !== activeTurnId)
            || (turnId && this.completedTurnIds.has(turnId))) return;
    ...
    private handleRawNotification(method: string, params: any): boolean {
        ...
        if (method === 'thread/status/changed') {
            // Thread status has no turn identity and can arrive late. Only a
            // matching turn completion can finish work.
            return true;
        }
        ...
        if (method === 'item/completed' && item.type === 'agentMessage') {
            ...
            // A decision message is not a terminal turn event.
            return true;
        }
    ...
    async connect(): Promise<void> {
        ...
        if (this.sandboxConfig?.enabled && process.platform !== 'win32') {
            try {
                ...
                const wrapped = await wrapForMcpTransport(codexExecutable(), ['app-server', '--listen', 'stdio://']);
        ...
        const proc = crossSpawn(command, args, {
            ...
            env,
            windowsHide: true,
            // npm's `codex` shim starts the native app-server as a child. Keep
            // that tree in its own process group so shutdown does not leave the
            // native server holding our stdio pipes after the shim exits.
            ...
            const killTimer = setTimeout(() => {
                try {
                    if (process.platform !== 'win32') {
                        process.kill(-pid, 0); // check process group alive
                        process.kill(-pid, 'SIGKILL');
                    } else {
                        process.kill(pid, 0); // check process alive
                        process.kill(pid, 'SIGKILL');
                    }
                } catch { /* already dead */ }
            }, 2000);
    ...
    async verifyConfigurationReady(): Promise<void> {
        if (!this._threadId) throw new Error('Original Codex session has not resumed');
        let cursor: string | undefined;
        let happyReady = false;
        do {
            const result = await this.request('mcpServerStatus/list', { threadId: this._threadId, detail: 'full', cursor }) as {
                data: Array<{ name: string; tools?: Record<string, unknown>; runtimeStatus?: string | null }>;
                nextCursor?: string | null;
            };
            for (const server of result.data) {
                const unhealthy = ['failed', 'starting', 'notStarted', 'authenticationRequired', 'cancelled'].includes(server.runtimeStatus ?? '');
                // Only LMC's own server gates the refresh. A third-party MCP that
                // is down (a plugin pointing at an app that is not installed on
                // this Mac) was already down for the original session; refusing to
                // refresh would pin that session to the old Agent forever.
                if (server.name === 'happy') {
                    if (unhealthy) throw new Error(`MCP ${server.name}: ${server.runtimeStatus}`);
                    if (Object.keys(server.tools ?? {}).length > 0) happyReady = true;
                } else if (unhealthy) {
                    logger.warn(`[CodexAppServer] MCP ${server.name} is ${server.runtimeStatus}; continuing without it`);
                }
            }
            cursor = result.nextCursor ?? undefined;
        } while (cursor);
        if (!happyReady) throw new Error('LMC session tools are not ready');
    }

    private buildThreadConfig(mcpServers?: Record<string, unknown>): Record<string, unknown> | null {
        const config: Record<string, unknown> = mcpServers ? { mcp_servers: mcpServers } : {};
        if (this.serviceTier !== undefined) config.service_tier = this.serviceTier;
        if (this.serviceTier === 'fast') config['features.fast_mode'] = true;
        if (this.contextLimits.contextWindow !== undefined) config.model_context_window = this.contextLimits.contextWindow;
        if (this.contextLimits.autoCompactTokenLimit !== undefined) config.model_auto_compact_token_limit = this.contextLimits.autoCompactTokenLimit;
    ...
    async resumeThread(opts?: {
        ...
        const params: ResumeConversationParams = {
            ...
            developerInstructions: null,
            persistExtendedHistory: true,
            // LMC already owns displayed history; avoid hydrating a large rollout
            // into thread.turns when only metadata and live-resume state are needed.
            excludeTurns: true,
        };
    ...
    async applyRuntimeConfiguration(limits: CodexContextLimits, tier?: CodexServiceTier): Promise<void> {
        const nextLimits = validateCodexContextLimits(limits);
        const nextTier = validateCodexServiceTier(tier);
        if (this.pendingTurnCompletion) throw new Error('Cannot reconfigure an active turn');
        const previousLimits = this.contextLimits;
        const previousTier = this.serviceTier;
        const threadId = this._threadId;
        this.contextLimits = nextLimits;
        ...
        try {
            ...
            this.contextLimits = previousLimits;
            this.serviceTier = previousTier;
            // Keep the identity even if reconnect failed: never silently start a new thread.
    ...
    async sendTurnAndWait(prompt: string, opts?: {
        ...
        // A turn can legitimately run for hours. A local deadline does not stop
        // Codex, so a default timeout would falsely release the queue and report
        // idle while the same turn is still producing output. Only explicit
        // callers opt into a deadline; RPC requests keep their own timeout.
        if (this.pendingTurnCompletion) {
            throw new Error('Cannot start a second turn while a turn is still active');
        }
    ...
    private async handleServerRequest(id: number, method: string, params: any): Promise<void> {
        if (method === 'mcpServer/elicitation/request') {
            const requiresInput = params?.mode === 'url'
                || Object.keys(params?.requestedSchema?.properties ?? {}).length > 0
                || (Array.isArray(params?.requestedSchema?.required) && params.requestedSchema.required.length > 0);
            const threadId = stringOrNull(params?.threadId) ?? this._threadId;
            ...
            const decision = await this.handleApproval({
                ...
                toolName,
                input: requiresInput ? { message: params.message, requestedSchema: params.requestedSchema, url: params.url, toolInput: params?._meta?.tool_params } : params?._meta?.tool_params ?? {},
                requiresInput,
                serverName,
        ...
        // Future approval methods must fail closed instead of receiving an ambiguous empty response.
        if (/requestApproval|approval|elicitation/i.test(method)) {
            logger.warn(`[CodexAppServer] Declining unknown approval request: ${method}`);
            this.respond(id, /elicitation/i.test(method)
    ...
    private handleNotification(method: string, params: any): void {
        ...
        if (lifecycle && this._threadId && threadId && threadId !== this._threadId) return;
        ...
        if (method === 'codex/event' || method.startsWith('codex/event/')) {
            ...
            if (msg) {
                const isLifecycle = ['task_started', 'task_complete', 'turn_aborted'].includes(msg.type);
                const isSubagent = ['subagent', 'parent_call_id', 'parentCallId', 'agent_thread_id', 'agentThreadId']
                    .some(key => typeof msg[key] === 'string' && msg[key].length > 0);
                const eventThread = threadId ?? params?.conversationId ?? msg.thread_id ?? msg.threadId;
                const eventTurn = msg.turn_id ?? msg.turnId;
                const activeTurn = this.pendingTurnCompletion?.turnId ?? this._turnId;
                if (isLifecycle && (isSubagent
                    || (eventThread && this._threadId && eventThread !== this._threadId)
                    || (msg.type !== 'task_started' && eventTurn && activeTurn && eventTurn !== activeTurn)
                    || (msg.type !== 'task_started' && eventTurn && this.completedTurnIds.has(eventTurn)))) return;
