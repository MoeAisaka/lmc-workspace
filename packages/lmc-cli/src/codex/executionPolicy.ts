...
export const CODEX_REMOTE_PERMISSION_MODES = [
    'auto',
    'default',
    'read-only',
    'safe-yolo',
...
export function isRemoteCodexPermissionMode(value: unknown): value is typeof CODEX_REMOTE_PERMISSION_MODES[number] {
    return typeof value === 'string'
        && (CODEX_REMOTE_PERMISSION_MODES as readonly string[]).includes(value);
}

export function resolveCodexExecutionPolicy(
    permissionMode: PermissionMode,
    sandboxManagedByLmc: boolean,
    hubGuarded = false,
): { approvalPolicy: ApprovalPolicy; sandbox: SandboxMode } {
    if (hubGuarded) return { approvalPolicy: 'never', sandbox: 'read-only' };
    if (sandboxManagedByLmc) {
        return {
...
/** A denial is final; undefined alone may enter the interactive permission UI. */
export function resolveCodexApprovalDecision(
    permissionMode: PermissionMode,
    sandboxManagedByLmc: boolean,
    { hubGuarded = false, latestPermissionMode }: { hubGuarded?: boolean; latestPermissionMode?: PermissionMode } = {},
): 'approved' | 'denied' | undefined {
    if (hubGuarded) return 'denied';
    if (shouldAutoApproveCodexApproval(latestPermissionMode ?? permissionMode, sandboxManagedByLmc)) {
        return 'approved';
    }
    return undefined;
}
