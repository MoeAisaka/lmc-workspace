...
export class CodexRemoteModeState {
    ...
    resolve(meta: MessageMeta | undefined): CodexRemoteModeResolution {
        // Validate the effective pair before committing ANY override, including
        // permission. A model-only switch must also check the inherited effort.
        const candidateModel = meta && Object.prototype.hasOwnProperty.call(meta, 'model')
            ? meta.model || undefined : this.currentModel;
        const candidateEffort = meta && Object.prototype.hasOwnProperty.call(meta, 'effort')
            ? meta.effort : this.currentEffort;
        assertCodexModelEffort(candidateModel, candidateEffort);
