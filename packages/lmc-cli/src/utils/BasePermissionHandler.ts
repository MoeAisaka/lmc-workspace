...
export interface PendingRequest {
    ...
    toolName: string;
    input: unknown;
    /** Set only by a native exec/patch approval, never inferred from a tool name or text. */
...
export abstract class BasePermissionHandler {
    ...
    protected setupRpcHandler(): void {
        this.session.rpcHandlerManager.registerHandler<PermissionResponse, void>(
            ...
            async (response) => {
                ...
                this.settlePending(response.id, pending, result);
    ...
    /** Claim the exact instance before resolving; a later answer cannot win twice. */
    protected settlePending(id: string, pending: PendingRequest, result: PermissionResult): boolean {
    ...
    protected approvePendingTools(): void {
        for (const [id, pending] of this.pendingRequests) {
            if (pending.policyAutoApprove) this.settlePending(id, pending, { decision: 'approved' });
        }
    }
