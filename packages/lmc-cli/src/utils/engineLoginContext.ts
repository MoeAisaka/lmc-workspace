...
export function engineLoginContext(engine: LoginEngine, cwd: string, env: NodeJS.ProcessEnv = process.env, args: string[] = []): EngineLoginContext {
    ...
    if (engine === 'claude') {
        ...
        try {
            const config = parse(readFileSync(resolve(configDir, 'config.toml'), 'utf8'));
            const table = (value: unknown): Record<string, unknown> | undefined =>
                value && typeof value === 'object' && !Array.isArray(value) ? value as Record<string, unknown> : undefined;
            // Inactive profiles/providers do not change the current credential scope.
            // The selected profile overrides root values; custom active auth still fails closed.
            const profile = typeof config.profile === 'string' ? table(table(config.profiles)?.[config.profile]) : undefined;
            if (config.profile !== undefined && !profile) supported = false;
            const effective = { ...config, ...profile };
            const provider = effective.model_provider ?? 'openai';
            const login = effective.forced_login_method;
            const providerConfig = table(table(config.model_providers)?.[String(provider)]);
            const customAuth = (value: Record<string, unknown> | undefined) =>
                ['auth_mode', 'env_key', 'api_key', 'base_url', 'experimental_bearer_token'].some(key => value?.[key] !== undefined);
            if (provider !== 'openai' || (login !== undefined && login !== 'chatgpt') || customAuth(effective) || customAuth(providerConfig)) supported = false;
        } catch (error: any) { if (error?.code !== 'ENOENT') supported = false; }
    }
