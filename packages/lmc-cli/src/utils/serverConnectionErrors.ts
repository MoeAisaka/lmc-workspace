    ...
    const attemptReconnect = async () => {
        ...
        try {
            ...
            if (axios.isAxiosError(e) && e.response?.status === 401) {
                ...
                config.onNotify('❌ Authentication failed. Please re-authenticate with `lmc auth`.');
...
export const ERROR_DESCRIPTIONS: Record<string, string> = {
    ...
    // HTTP errors
    '401': 'authentication failed - run `lmc auth`',
...
class OfflineState {
    ...
    private print(): void {
        const summary = [...this.failures.values()]
            ...
            .join('; ');
        console.log(`⚠️  LMC server unreachable, offline mode with auto-reconnect enabled - error details: ${summary}`);
