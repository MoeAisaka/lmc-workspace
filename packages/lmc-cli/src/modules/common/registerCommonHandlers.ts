import { registerResourceHandlers } from './resourceFiles';
...
export interface SpawnSessionOptions {
     ...
     * LMC session continues from an existing Claude conversation file.
    ...
    /** Start the session bound as this hub's worker (auto binding, written at birth). */
    hubSessionId?: string;
    /** Initial title, duty prefix included: 【编码】…  */
    title?: string;
...
export function registerCommonHandlers(rpcHandlerManager: RpcHandlerManager, workingDirectory: string | null) {
    ...
    /**
     * Reads are not confined to the project. The same authenticated client can
     * spawn a session that runs arbitrary shell on this Mac, so refusing to
     * *show* a file it can already `cat` buys no safety — it only breaks
     * opening a tool's output when the agent touched /tmp or a sibling repo.
     * Writes keep the containment check.
     */
    const checkReadPath = (targetPath: string): PathValidationResult =>
        ({ valid: true, resolvedPath: resolve(workingDirectory ?? '.', targetPath) });
    ...
    rpcHandlerManager.registerHandler<ReadFileRequest, ReadFileResponse>('readFile', async (data) => {
        ...
        const validation = checkReadPath(data.path);
        if (!validation.valid) {
            return { success: false, error: validation.error };
        }
