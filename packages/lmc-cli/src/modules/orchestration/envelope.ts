 ...
 *   verification …
 *   questions   …
 *   cost        (the worker's runner: turns and tokens since dispatch)
 *   evidence    (explicit business evidence supplied by the worker)
 *   trace       (the runner's automatic transcript tail, not report identity)
 *
 *   [review lmc-42 · attempt 1 · accepted]
 *   summary     …
 *   reasons     …
 *
 * A review is the hub's (or the person's) verdict on a report: `accepted`
 * closes the task, `rejected` sends it back for another attempt.
 */

export const TASK_FIELDS = ['dispatch', 'stage', 'goal', 'scope', 'acceptance', 'constraints', 'deliver', 'run', 'budget'] as const;
...
export const REPORT_FIELDS = ['dispatch', 'digest', 'summary', 'changes', 'verification', 'questions', 'blocked', 'cost', 'model', 'evidence', 'trace'] as const;
export const REPORT_STATUSES = ['done', 'blocked', 'failed'] as const;
export const REVIEW_FIELDS = ['dispatch', 'summary', 'reasons'] as const;
...
/**
 * Bound the hub's context. Evidence and the automatic trace may run longer;
 * business identity is computed before these display limits are applied.
 */
...
export function parseEnvelope(text: string, bounded = true): Envelope | null {
    ...
    for (const key of Object.keys(fields)) {
        ...
        if (bounded) fields[key] = fields[key].slice(0, key === 'evidence' || key === 'trace' ? ENVELOPE_LIMITS.evidence : ENVELOPE_LIMITS.field);
    }
    if (kind === 'task') return { kind, id, attempt, fields };
    if (kind === 'review') {
        const verdict = statusMatch?.[1].toLowerCase();
        return { kind: 'review', id, attempt, status: verdict === 'rejected' ? 'rejected' : 'accepted', fields };
    }
    const statusWord = statusMatch?.[1]?.toLowerCase();
    const knownStatus = !!statusWord && (REPORT_STATUSES as readonly string[]).includes(statusWord);
    if (!knownStatus && !fields.blocked) {
        // The head has a word here this format does not recognise as a status
        // (an acknowledgement like "ack", a typo, anything but done/blocked/
        // failed), and nothing in the fields says blocked either. Defaulting
        // to 'done' would let a made-up word silently close a task on the
        // board; safer to say this was not a [report …] envelope at all and
        // let it fall through as ordinary mail.
        const residual = extras.replace(/(?:attempt|#)\s*\d{1,3}/i, '').replace(/[\s·|,-]+/g, '').trim();
        if (residual) return null;
    }
    const status = (knownStatus ? statusWord! : (fields.blocked ? 'blocked' : 'done')) as ReportStatus;
