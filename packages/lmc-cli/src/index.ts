...
import { checkIfDaemonRunningAndCleanupStaleState, isDaemonRunningCurrentlyInstalledLmcVersion, stopDaemon } from './daemon/controlClient'
import { getLatestDaemonLog } from './ui/logger'
import { killRunawayLmcProcesses } from './daemon/doctor'
  ...
  if (subcommand === 'doctor') {
    ...
    if (args[1] === 'clean') {
      if (args.slice(2).some(a => a === '--help' || a === '-h')) {
        console.log(`
${chalk.bold('lmc doctor clean')} - Kill all LMC-related processes (daemon + sessions)
...
`)
      ...
      const result = await killRunawayLmcProcesses()
    ...
    console.log('LMC 中心使用独立 MySQL 部署，请参考 deploy/lmc/README.md。');
    return;
    ...
    // Handle "lmc gemini model set <model>" command
    ...
    // Handle "lmc gemini model get" command
    ...
    // Handle "lmc gemini project set <project-id>" command
    if (geminiSubcommand === 'project' && args[2] === 'set' && args[3]) {
      ...
      try {
        ...
        // Try to get current user email from the LMC center token
    ...
    // Handle "lmc gemini project get" command
    if (geminiSubcommand === 'project' && args[2] === 'get') {
      try {
        ...
        if (config.googleCloudProject) {
          ...
          console.log('  lmc gemini project set <your-project-id>');
          console.log('');
    ...
    // Handle "lmc gemini project" (no subcommand) - show help
    if (geminiSubcommand === 'project' && !args[2]) {
      console.log('Usage: lmc gemini project <command>');
      console.log('');
    ...
    try {
      ...
      console.warn(chalk.yellow('⚠ The gemini backend is deprecated and may be removed in a future release. Use `lmc agy` (Antigravity CLI) instead.'));
    ...
    console.log(chalk.yellow('Note: "lmc logout" is deprecated. Use "lmc auth logout" instead.\n'));
    try {
    ...
    if (daemonSubcommand === 'list') {
      ...
      const child = spawnLmcCLI(['daemon', 'start-sync'], {
        detached: true,
        stdio: 'ignore',
      ...
      console.log(`
${chalk.bold('lmc daemon')} - Daemon management

${chalk.bold('Usage:')}
  lmc daemon start              Start the daemon (detached)
  lmc daemon stop               Stop the daemon (sessions stay alive)
  lmc daemon status             Show daemon status
  lmc daemon list               List active sessions

  If you want to kill all happy related processes run 
  ${chalk.cyan('lmc doctor clean')}

${chalk.bold('Note:')} The daemon runs in the background and manages Claude sessions.

${chalk.bold('To clean up runaway processes:')} Use ${chalk.cyan('lmc doctor clean')}
`)
    }
    return;
  } else {
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      if (arg === '-h' || arg === '--help') {
        ...
        // LMC-specific flag to disable chrome even if default is on
      } else if (arg === '--settings') {
        // Intercept --settings flag - LMC uses this internally for session hooks
        const settingsValue = args[++i] // consume the value
        console.warn(chalk.yellow(`⚠️  Warning: --settings is used internally by LMC for session tracking.`))
    ...
    if (showHelp) {
      console.log(`
${chalk.bold('lmc')} - Claude Code On the Go

${chalk.bold('Usage:')}
  lmc [options]         Start Claude with mobile control
  lmc auth              Manage authentication
  lmc resume            Resume a previous LMC session by LMC session ID
  lmc codex             Start Codex mode
  lmc gemini            Start Gemini mode (ACP) [deprecated — use agy]
  lmc agy               Start agy (Antigravity CLI) mode
  lmc acp               Start a generic ACP-compatible agent
  lmc connect           Connect AI vendor API keys
  lmc sandbox           Configure and manage OS-level sandboxing
  lmc notify            Send push notification
  lmc daemon            Manage background service that allows
                            to spawn new sessions away from your computer
  lmc doctor            System diagnostics & troubleshooting

${chalk.bold('Examples:')}
  happy                    Start session
  lmc resume cmmij8      Resume a previous session by LMC session ID
  lmc --yolo             Start with bypassing permissions
                            happy sugar for --dangerously-skip-permissions
  lmc --chrome           Enable Chrome browser access for this session
  lmc --no-chrome        Disable Chrome even if default is on
  lmc --no-sandbox       Disable LMC sandbox for this session
  lmc --js-runtime bun   Use bun instead of node to spawn Claude Code
  lmc --claude-env ANTHROPIC_BASE_URL=http://127.0.0.1:3456
                           Use a custom API endpoint (e.g., claude-code-router)
  lmc acp gemini         Start Gemini via generic ACP runner
  lmc acp -- opencode --acp
                           Start a custom ACP command
  lmc acp opencode --verbose
                           Print raw ACP backend/envelope events
  lmc auth login --force Authenticate
  lmc doctor             Run diagnostics

${chalk.bold('LMC supports ALL Claude options!')}
...
`)
    ...
    if (showVersion) {
      ...
      return; // A version probe must not authenticate or register a session.
    }
...
async function handleNotifyCommand(args: string[]): Promise<void> {
  ...
  if (showHelp) {
    console.log(`
${chalk.bold('lmc notify')} - Send notification

${chalk.bold('Usage:')}
  lmc notify -p <message> [-t <title>]    Send notification with custom message and optional title
  lmc notify -h, --help                   Show this help
...
${chalk.bold('Examples:')}
  lmc notify -p "Deployment complete!"
  lmc notify -p "System update complete" -t "Server Status"
  lmc notify -t "Alert" -p "Database connection restored"
`)
    return
  }
  ...
  if (!message) {
    ...
    console.log(chalk.gray('Run "lmc notify --help" for usage information.'))
    process.exit(1)
  }
  ...
  if (!credentials) {
    console.error(chalk.red('Error: Not authenticated. Please run "lmc auth login" first.'))
    process.exit(1)
  }
  ...
  try {
    ...
    // Use custom title or default to "LMC"
    const notificationTitle = title || 'LMC'
