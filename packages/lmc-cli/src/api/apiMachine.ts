...
/**
 * WebSocket client for machine/daemon communication with LMC server
...
type MachineRpcHandlers = {
    ...
    resumeSession?: (sessionId: string, options?: { model?: string; permissionMode?: string; codexServiceTier?: CodexServiceTier; codexContextLimits?: CodexContextLimits }) => Promise<SpawnSessionResult>;
...
export class ApiMachineClient {
    ...
    private resumeSessionHandler: ((sessionId: string, options?: { model?: string; permissionMode?: string; codexServiceTier?: CodexServiceTier; codexContextLimits?: CodexContextLimits }) => Promise<SpawnSessionResult>) | null = null;
    private reconnectInterval: NodeJS.Timeout | null = null;
    private declaredMetadata: Partial<MachineMetadata> | null = null;
    ...
    registerDeviceHandler(method: string, handler: (params: any) => Promise<any>) {
        this.rpcHandlerManager.registerHandler(method, handler);
    }

    async callSession(sessionId: string, key: Uint8Array, variant: 'legacy' | 'dataKey', method: string, params: unknown): Promise<any> {
        if (!this.socket.connected) throw new Error('设备连接已断开');
        const result = await this.socket.timeout(20000).emitWithAck('rpc-call', {
            method: `${sessionId}:${method}`, params: encodeBase64(encrypt(key,variant,params)),
        });
        if(!result.ok) throw new Error(result.error || '会话未响应');
        const data=decrypt(key,variant,decodeBase64(result.result)) as any;
    ...
    setRPCHandlers({
        ...
        // The method was named `spawn-happy-session` before the rename. Register
        // both spellings against one handler so a caller on either side of the
        // rename reaches the same code: an app that has switched to the new name
        // must not fail against a daemon that has not, and vice versa. Drop the
        // old registration only once every device runs a version that has this.
        const spawnSessionHandler = async (params: any) => {
            const { directory, sessionId, machineId, approvedNewDirectoryCreation, agent, permissionMode, modelMode, effortLevel, codexServiceTier, codexContextLimits, environmentVariables, token, resumeClaudeSessionId, resumeCodexThreadId, parentSessionId, forkedFromMessageId, isSideChat, hubSessionId, title } = params || {};
            ...
            if (agent === 'codex') { validateCodexServiceTier(codexServiceTier); validateCodexContextLimits(codexContextLimits); }
            const result = await spawnSession({ directory, sessionId, machineId, approvedNewDirectoryCreation, agent, permissionMode, modelMode, effortLevel, codexServiceTier, codexContextLimits, environmentVariables, token, resumeClaudeSessionId, resumeCodexThreadId, parentSessionId, forkedFromMessageId, isSideChat, hubSessionId: typeof hubSessionId === 'string' ? hubSessionId : undefined, title: typeof title === 'string' ? title : undefined });
        ...
        this.rpcHandlerManager.registerHandler('spawn-lmc-session', spawnSessionHandler);
        ...
        // Claude UUID. The caller then spawns a fresh LMC session with
        ...
        if (this.resumeSessionHandler) {
            if (!this.rpcHandlerManager.hasHandler(method)) {
                this.rpcHandlerManager.registerHandler(method, async (params: any) => {
                    const { sessionId, model, permissionMode, codexServiceTier, codexContextLimits } = params || {};
                    ...
                    validateCodexServiceTier(codexServiceTier);
                    if (codexContextLimits !== undefined) validateCodexContextLimits(codexContextLimits);
                    const result = await handler(sessionId, { model, permissionMode, codexServiceTier, codexContextLimits });
                    switch (result.type) {
                        case 'success':
                            return { type: 'success', sessionId: result.sessionId };
                        case 'requestToApproveDirectoryCreation':
    ...
    /**
     * Static facts the daemon knows about itself (capabilities, version, paths).
     * Registration only creates a machine; the server hands an existing one back
     * untouched, so a daemon upgrade never reaches the app unless we merge these
     * in ourselves on every connect.
     */
    declareMetadata(values: Partial<MachineMetadata>) {
        this.declaredMetadata = values;
    }

    private async syncDeclaredMetadata() {
        const declared = this.declaredMetadata;
        if (!declared) return;
        const current = (this.machine.metadata || {}) as Record<string, unknown>;
        const stale = Object.keys(declared).some(key => JSON.stringify(current[key]) !== JSON.stringify((declared as Record<string, unknown>)[key]));
        if (!stale) return;
        try {
            await this.updateMachineMetadata(metadata => ({ ...(metadata || {} as any), ...declared }));
        } catch (error) {
            logger.debug('[API MACHINE] Failed to publish declared metadata:', error);
        }
    }
    ...
    connect() {
        ...
        this.socket = io(serverUrl, {
            ...
            auth: {
                ...
                machineId: this.machine.id,
                lmcClient: `cli-daemon/${configuration.currentCliVersion}`
            },
            path: '/v1/updates',
            reconnection: false,
        });
        ...
        this.socket.on('connect', () => {
            ...
            void this.syncDeclaredMetadata();
