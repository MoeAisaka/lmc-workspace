...
describe('Api server error handling', () => {
    ...
    describe('getOrCreateSession', () => {
        it('should return null when the LMC server is unreachable (ECONNREFUSED)', async () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith(
                expect.stringContaining('⚠️  LMC server unreachable')
            );

            consoleSpy.mockRestore();
        });

        it('should return null when the LMC server cannot be found (ENOTFOUND)', async () => {
        ...
        it('should return null when the LMC server times out (ETIMEDOUT)', async () => {
            connectionState.reset();
            const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
