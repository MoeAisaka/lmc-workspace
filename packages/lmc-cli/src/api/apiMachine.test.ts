...
const publishedMetadata = (socket: any) => socket.emitWithAck.mock.calls
    .filter(([event]: [string]) => event === 'machine-update-metadata')
    .map(([, body]: [string, any]) => decrypt(new Uint8Array(32), 'legacy', decodeBase64(body.metadata)));
...
describe('ApiMachineClient socket reconnection', () => {
    ...
    it('publishes declared metadata on connect when the server copy is stale', async () => {
        mockSocket.emitWithAck.mockImplementation(() => new Promise(() => {}));
        const client = new ApiMachineClient('fake-token', makeMachine());
        client.declareMetadata({ managedUpgrades: true, happyCliVersion: '1.2.3' });
        client.connect();
        emitSocketEvent('connect');
        await Promise.resolve();
        // The capability refresh publishes too; only the declared merge carries these keys.
        const declared = publishedMetadata(mockSocket).filter((m: any) => m.managedUpgrades === true);
        expect(declared).toHaveLength(1);
        expect(declared[0]).toEqual(expect.objectContaining({ host: 'localhost', happyCliVersion: '1.2.3', managedUpgrades: true }));
        client.shutdown();
    });

    it('leaves metadata alone when every declared value already matches', async () => {
        mockSocket.emitWithAck.mockImplementation(() => new Promise(() => {}));
        const client = new ApiMachineClient('fake-token', makeMachine());
        client.declareMetadata({ host: 'localhost', happyCliVersion: 'test' });
        client.connect();
        emitSocketEvent('connect');
        await Promise.resolve();
        expect(publishedMetadata(mockSocket).some((m: any) => 'managedUpgrades' in m || m.happyCliVersion !== 'test')).toBe(false);
        client.shutdown();
    });
