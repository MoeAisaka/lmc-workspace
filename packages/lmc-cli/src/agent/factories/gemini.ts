...
export interface GeminiBackendOptions extends AgentFactoryOptions {
  ...
  /** OAuth token from the LMC center (via 'lmc connect gemini') - highest priority */
  cloudToken?: string;
...
export function createGeminiBackend(options: GeminiBackendOptions): GeminiBackendResult {
  ...
  // 1. the LMC center OAuth token (via 'lmc connect gemini') - highest priority
  ...
  let apiKey = options.cloudToken       // 1. LMC center token (passed from runGemini)
  ...
  if (!apiKey) {
    logger.warn(`[Gemini] No API key found. Run 'lmc connect gemini' to authenticate via Google OAuth, or set ${GEMINI_API_KEY_ENV} environment variable.`);
  }
