...
describe('resolveAcpAgentConfig', () => {
  ...
  it('throws with helpful usage when no args are provided', () => {
    expect(() => resolveAcpAgentConfig([])).toThrow('Usage: lmc acp <agent-name> or lmc acp -- <command> [args]');
  });

  it('throws when separator form omits command', () => {
    expect(() => resolveAcpAgentConfig(['--'])).toThrow('Missing command after "--". Usage: lmc acp -- <command> [args]');
  });
});
