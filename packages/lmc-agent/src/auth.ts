...
export async function authLogin(config: Config): Promise<void> {
    ...
    // A browser link on this centre's own origin, not a custom scheme: the web app
    // handles it with no OS registration, which is what lets account pairing leave
    // the `happy://` name at all. The key rides in the fragment, so it never
    // reaches the server. Older apps that only read `happy://` are why the app
    // still accepts both.
    const qrData = `${config.serverUrl.replace(/\/$/, '')}/account/connect#key=${encodeBase64Url(keypair.publicKey)}`;
    console.log('');
    qrcode.generate(qrData, { small: true }, (code: string) => {
        console.log(code);
    });
    console.log('## Authentication');
    console.log('- Action: Scan this QR code, or open the link, with LMC');
