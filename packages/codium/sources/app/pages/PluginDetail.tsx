...
import { lmcClient, useLmcState } from '@/lmc/client'
...
function LmcAuthPanel() {
    const state = useLmcState()
    ...
    if (state.status === 'starting') {
        return <small className="plugin-detail__help">Loading LMC state…</small>
    }
            ...
            <div className="plugin-detail__row">
                ...
                <div className="plugin-detail__actions">
                    <button
                        type="button"
                        className="plugins-page__action"
                        onClick={() => run('logout', () => lmcClient.logout())}
                        disabled={busy !== null}
                    >
                        Disconnect
                    </button>
    ...
    if (state.status === 'authenticating' && authUrl) {
        return (
            <div className="plugin-detail__row plugin-detail__row--column">
                <div className="lmc-auth__qr-wrap">
                    {qrDataUrl ? (
                        <img className="lmc-auth__qr" src={qrDataUrl} alt="LMC authentication QR code" />
                    ) : (
                        <div className="lmc-auth__qr lmc-auth__qr--empty" />
                    )}
                </div>
                <div className="lmc-auth__url-row">
                    <input
                        className="plugin-detail__input lmc-auth__url"
                        value={authUrl}
                        readOnly
                        spellCheck={false}
                    />
                ...
                <div className="plugin-detail__actions">
                    <button
                        type="button"
                        className="plugins-page__action"
                        onClick={() => run('cancel', () => lmcClient.cancelAuth())}
                        disabled={busy !== null}
                    >
                        Cancel
                    </button>
        ...
        <div className="plugin-detail__row plugin-detail__row--column">
            ...
            <div className="plugin-detail__actions">
                <button
                    type="button"
                    className="plugins-page__action plugins-page__action--primary"
                    onClick={() => run('create', () => lmcClient.createAccount())}
                    disabled={busy !== null}
                >
                ...
                <button
                    type="button"
                    className="plugins-page__action"
                    onClick={() => run('link', () => lmcClient.startLinkDevice())}
                    disabled={busy !== null}
                >
            ...
            <div className="lmc-auth__url-row">
                ...
                <button
                    type="button"
                    className="plugins-page__action"
                    onClick={() => run('restore', () => lmcClient.restoreSecret(secretKey))}
