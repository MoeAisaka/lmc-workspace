...
const lmc = {
    getState: (): Promise<LmcStateSnapshot> =>
        ipcRenderer.invoke('lmc:state:get'),
    createAccount: (): Promise<LmcStateSnapshot> =>
        ipcRenderer.invoke('lmc:create-account'),
    startLinkDevice: (): Promise<LmcStateSnapshot> =>
        ipcRenderer.invoke('lmc:start-link-device'),
    restoreSecret: (secretKey: string): Promise<LmcStateSnapshot> =>
        ipcRenderer.invoke('lmc:restore-secret', secretKey),
    cancelAuth: (): Promise<LmcStateSnapshot> =>
        ipcRenderer.invoke('lmc:cancel-auth'),
    logout: (): Promise<LmcStateSnapshot> =>
        ipcRenderer.invoke('lmc:logout'),
    clientStatus: (): Promise<LmcAuthenticatedClientStatus> =>
        ipcRenderer.invoke('lmc:client-status'),
    onState(cb: (state: LmcStateSnapshot) => void) {
        const listener = (_: unknown, state: LmcStateSnapshot) => cb(state)
        ipcRenderer.on('lmc:state', listener)
        return () => ipcRenderer.off('lmc:state', listener)
    },
}
...
if (process.contextIsolated) {
    try {
        ...
        contextBridge.exposeInMainWorld('lmc', lmc)
