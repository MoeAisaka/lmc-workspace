...
if (!parentPort) {
    throw new Error('lmc worker must be started via worker_threads')
}
...
const send = (m: LmcWorkerMessage) => port.postMessage(m)

let credentials: StoredCredentials | null = null
let client: LmcAuthenticatedClient | null = null
let linkFlow: LinkFlow | null = null
let state: LmcStateSnapshot = {
    status: 'starting',
...
port.on('message', (msg: LmcWorkerRequestWithId) => {
    Promise.resolve()
        .then(() => handle(msg))
        .catch((err) => {
...
async function boot(): Promise<void> {
    try {
        ...
        client = credentials ? await LmcAuthenticatedClient.create(config, credentials) : null
        setState(snapshotForCredentials(credentials))
    } catch (err) {
        setError(`Failed to load LMC credentials: ${errString(err)}`)
    }
}

async function handle(msg: LmcWorkerRequestWithId): Promise<void> {
        ...
        try {
            ...
            if (data.state === 'authorized') {
                if (typeof data.token !== 'string' || typeof data.response !== 'string') {
                    throw new Error('LMC auth response is missing token data')
                }
                const encrypted = decodeBase64(data.response)
                const secret = decryptBox(encrypted, flow.keypair.secretKey)
                if (!secret || secret.length !== 32) {
                    throw new Error('Failed to decrypt LMC account credentials')
                }
                linkFlow = null
            ...
            if (!flow.cancelled && linkFlow === flow) {
                linkFlow = null
                setError(`LMC link failed: ${errString(err)}`)
            }
            return
        }
        await delay(1000)
...
async function storeAuthenticated(token: string, secret: Uint8Array): Promise<void> {
    ...
    client = await LmcAuthenticatedClient.create(config, credentials)
    setState(snapshotForCredentials(credentials))
}

async function createEncryptionContext(masterSecret: Uint8Array): Promise<LmcEncryptionContext> {
...
async function authGetToken(secret: Uint8Array): Promise<string> {
    ...
    if (typeof data.token !== 'string') {
        throw new Error('LMC authentication did not return a token')
    }
    return data.token
}
...
async function apiJson(path: string, body: unknown): Promise<Record<string, unknown>> {
    ...
    if (!response.ok) {
        ...
        throw new Error(`LMC API ${response.status}: ${text || response.statusText}`)
    }
...
function snapshotForCredentials(auth: StoredCredentials | null): LmcStateSnapshot {
    if (!auth) {
        return {
            ...stateBase(),
            status: 'unconfigured',
...
function setState(next: LmcStateSnapshot): void {
    state = next
...
function respond(requestId: string, next: LmcStateSnapshot, value?: unknown): void {
