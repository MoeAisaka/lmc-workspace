...
import { View, Text, Animated, Platform, Switch } from 'react-native';
...
import { switchSessionEngine } from '@/sync/sessionConfiguration';
import { sessionArchive, sessionKill, sessionDelete, sessionSetAgentMail } from '@/sync/ops';
...
function SessionInfoContent({ session }: { session: Session }) {
    ...
    const [deletingSession, performDelete] = useLmcAction(async () => {
        // Prompt for worktree cleanup before killing (needs an active machine connection)
        await maybeCleanupWorktree(session.id, session.metadata?.path, session.metadata?.machineId);
        ...
        if (!result.success) {
            throw new LmcError(result.message || t('sessionInfo.failedToDeleteSession'), false);
        }
    });
            ...
            >
                ...
                <ItemGroup>
                    <Item
                        ...
                        icon={<Ionicons name="finger-print-outline" size={29} color="#0060F0" />}
                    ...
                    <Item
                        ...
                        icon={<Ionicons name="calendar-outline" size={29} color="#0060F0" />}
                        showChevron={false}
                    />
                    <Item
                        title={t('sessionInfo.lastUpdated')}
                        subtitle={formatDate(session.updatedAt)}
                        icon={<Ionicons name="time-outline" size={29} color="#0060F0" />}
                        showChevron={false}
                    />
                    <Item
                        title={t('sessionInfo.sequence')}
                        detail={session.seq.toString()}
                        icon={<Ionicons name="git-commit-outline" size={29} color="#0060F0" />}
                        showChevron={false}
                    />
                </ItemGroup>

                <ItemGroup title={t('lmc.mail.section')} footer={t('lmc.mail.footer')}>
                    <Item
                        title={t('lmc.mail.accept')}
                        icon={<Ionicons name="mail-outline" size={29} color="#0060F0" />}
                        showChevron={false}
                        rightElement={
                            <Switch
                                value={session.metadata?.agentMail !== false}
                                onValueChange={(next: boolean) => {
                                    sessionSetAgentMail(session.id, next).catch(() => {
                                        Modal.alert(t('lmc.mail.failed'), t('lmc.menu.retryHint'));
                                    });
                                }}
                            />
                        }
                ...
                <CollaborationGroup sessionId={session.id} />
                ...
                <ItemGroup title={t('sessionInfo.quickActions')}>
                    <Item
                        ...
                        icon={<Octicons name="file-diff" size={26} color="#0060F0" />}
                    ...
                    {session.metadata?.machineId && (
                        <Item
                            ...
                            icon={<Ionicons name="server-outline" size={29} color="#0060F0" />}
                    ...
                    {canShowResume && (
                        <Item
                            ...
                            icon={<Ionicons name="play-circle-outline" size={29} color="#0060F0" />}
                ...
                {session.metadata && (
                    <ItemGroup title={t('sessionInfo.metadata')}>
                        ...
                        {session.metadata.engineRuntime && <Item title={t('lmc.sessionInfo.actualEngine')} subtitle={`${session.metadata.engineRuntime.engine} ${session.metadata.engineRuntime.version} · ${session.metadata.engineRuntime.source === 'lmc-managed' ? t('lmc.sessionInfo.managedVersion') : session.metadata.engineRuntime.source === 'sdk-bundled' ? t('lmc.sessionInfo.sdkVersion') : t('lmc.sessionInfo.systemVersion')}`} showChevron={false}/>}
                        {session.metadata.agentBuild && <Item title={t('lmc.sessionInfo.agentBuild')} subtitle={session.metadata.agentBuild.slice(0,12)} showChevron={false}/>}
                        {session.metadata.os && (
                            <Item
                        ...
                        <Item
                            ...
                            icon={<Ionicons name="copy-outline" size={29} color="#0060F0" />}
                            onPress={handleCopyMetadata}
                        />
                        <Item
                            title={t('sessionInfo.copyMetadata') + '\n& Client Logs'}
                            icon={<Ionicons name="document-text-outline" size={29} color="#0060F0" />}
                        ...
                        {sessionStatus && (
                            <>
                                <Item
                                    ...
                                    icon={<Ionicons name="analytics-outline" size={29} color="#0060F0" />}
                                    showChevron={false}
                                />
                                <View style={{ marginHorizontal: 16, marginBottom: 12 }}>
                                    <CodeView 
...
/**
 * The session's place among hubs and workers, folded into one row: its
 * detail names the current role, and opening it reaches the full
 * CollaborationSheet — the same sheet a worker uses to move to a new hub
 * when this one's is lost.
 */
function CollaborationGroup({ sessionId }: { sessionId: string }) {
    const sessions = useAllSessions();
    const me = sessions.find((s) => s.id === sessionId);
    const orchestration = me?.metadata?.orchestration;
    const hub = orchestration?.role === 'worker' ? sessions.find((s) => s.id === orchestration.hub.sessionId) : null;
    // A hub that is archived, gone from the list, or offline for good has left
    // its workers with nobody to report to.
    const hubLost = orchestration?.role === 'worker' && (!hub || hub.metadata?.lifecycleState === 'archived');
    const detail = orchestration?.role === 'hub'
        ? t('lmc.orchestration.hub')
        : orchestration?.role === 'worker'
            ? (hubLost ? t('lmc.orchestration.hubLost') : t('lmc.orchestration.belongsTo', { name: hub ? getSessionName(hub) : orchestration.hub.sessionId.slice(0, 8) }))
            : t('lmc.orchestration.plain');
    return (
        <ItemGroup>
            <Item
                title={t('lmc.orchestration.section')}
                detail={detail}
                icon={<Ionicons name={hubLost ? 'alert-circle-outline' : 'disc-outline'} size={29} color={hubLost ? '#E08A00' : '#0060F0'} />}
