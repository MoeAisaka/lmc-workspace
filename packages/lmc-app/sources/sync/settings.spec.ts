...
describe('settings', () => {
    ...
    describe('settingsDefaults', () => {
        ...
        it('carries a retired setting through as an unknown field instead of dropping it', () => {
            // The harness-icon switches are gone; a profile saved by an older
            // build must still parse, and must not resurrect them as defaults.
            const parsed = settingsParse({ showFlavorIcons: true }) as Record<string, unknown>;
            expect(parsed.showHarnessIconInSessionHeader).toBeUndefined();
            expect(parsed.experiments).toBe(settingsDefaults.experiments);
        });
