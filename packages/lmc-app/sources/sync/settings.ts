...
export const SettingsSchema = z.object({
    ...
    sessionListGroupByEngine: z.boolean().describe('Sidebar list: nest sessions under their engine within each device, or list them directly under the device with the engine as a badge'),
    ...
    projectAvatarOverrides: z.record(z.string(), z.object({
        uri: z.string().max(140000).regex(/^data:image\/(png|jpeg);base64,[A-Za-z0-9+/]+={0,2}$/),
        thumbhash: z.string().max(200),
    }).nullable()),
    sessionProjectOrder: z.record(z.string(), z.array(z.string()).nullable()),
    sessionNameOverrides: z.record(z.string(), z.string().max(200).nullable()),
...
export function applySettings(settings: Settings, delta: Partial<Settings>): Settings {
    ...
    if (delta.projectAvatarOverrides) result.projectAvatarOverrides = { ...settings.projectAvatarOverrides, ...delta.projectAvatarOverrides };
    if (delta.sessionProjectOrder) result.sessionProjectOrder = { ...settings.sessionProjectOrder, ...delta.sessionProjectOrder };
    if (delta.sessionNameOverrides) {
        result.sessionNameOverrides = { ...settings.sessionNameOverrides, ...delta.sessionNameOverrides };
    }
