 ...
 *
 * LMC gives every daemon a machine of its own, so a computer running both LMC CLI and Happy
...
export interface MachineChoice {
    /**
     * What a draft stores, which is LMC CLI's machine whenever this computer has one.
     *
    ...
    machineIds: string[];
    /** The daemon LMC CLI runs here, if it runs one. */
    lmcMachine: Machine | null;
...
function getComputerName(choice: { lmcMachine: Machine | null; rigMachine: Machine | null }): string {
    const named = choice.lmcMachine ?? choice.rigMachine;
    ...
    if (choice.lmcMachine === null && named.metadata?.host) return named.metadata.host;
 ...
 * registrations naming the same LMC CLI are the same laptop, but neither names the other. Asking
 * one hop from whichever machine was seen first splits that laptop in two and puts the LMC CLI
...
export function collectMachineChoices(machines: readonly Machine[]): MachineChoice[] {
    ...
    for (const machine of machines) {
        ...
        // different data directory mints a new machine identity and claims the same LMC CLI
        ...
        const lmcMachine = preferLiveliest(group.filter((member) => !isRigMachine(member.metadata)));
        choices.push({
            id: (lmcMachine ?? rigMachine ?? machine).id,
            name: getComputerName({ lmcMachine, rigMachine }),
            machineIds: ids,
            lmcMachine,
            rigMachine,
 ...
 *
 * A computer with no LMC CLI daemon cannot run Claude Code however much the picker would like
...
export function machineChoiceAgentAvailable(
    ...
    if (agent === 'rig') {
        return isHarnessAvailable({
            availability: choice.lmcMachine?.metadata?.cliAvailability,
            happyAgentAvailable: choice.rigMachine !== null,
            key: agent,
        });
    }
    const happy = choice.lmcMachine;
...
export function resolveChoiceAgent(
    ...
    if (machineChoiceAgentAvailable(choice, agent)) {
        // Older LMC CLI machines are trusted for common harnesses. Antigravity
        // never reaches this branch without an explicit installation report.
        return agent === 'rig' || !choice.lmcMachine?.metadata?.cliAvailability
            ? agent
            : resolveMachineAgent(agent, choice.lmcMachine.metadata.cliAvailability);
    }
...
export function resolveAgentMachine(
    ...
    return agent === 'rig' ? choice.rigMachine : choice.lmcMachine;
}
...
export function resolveWorktreeCreationMachine(
    ...
    if (agent === 'rig') {
        const lmcMachine = choice.lmcMachine;
        if (lmcMachine && isMachineOnline(lmcMachine)) {
            return lmcMachine;
        }
    }
