...
const agentEventSchema = z.discriminatedUnion('type', [z.object({
    ...
    // One engine handed this session to another. `briefing` is what actually
    // crossed, kept here so the reader can check what the next engine was told.
    type: z.literal('engine-handoff'),
    from: z.string(),
    /** The engine key, for deciding how to render what came before this point. */
    fromFlavor: z.enum(['claude', 'codex']).optional(),
    source: z.enum(['engine', 'compiled']),
    briefing: z.string(),
}), z.object({
    // A queued switch was called off before the relaunch. The switch card
    // reads it; nothing is drawn for it on its own.
    type: z.literal('engine-switch-cancelled'),
...
export type NormalizedMessage = ({
    ...
    claudeUuid?: string,
    codexItemId?: string,
    /** Server-side sequence while this decrypted message is resident locally. */
    serverSeq?: number,
    /** Main/subagent envelope turn identity for lifecycle deduplication. */
    turnId?: string,
};

function normalizeSessionEnvelope(envelope: SessionEnvelope, localId: string | null, createdAt: number, meta: MessageMeta | undefined): NormalizedMessage | null {
    const normalized = normalizeSessionEnvelopeContent(envelope, localId, createdAt, meta);
    return normalized ? { ...normalized, turnId: envelope.turn } : null;
}
