...
interface SessionMessages {
    ...
    // Decrypted source messages retained only on web. This bounded window is
    // the complete replay source for the reducer after an edge is evicted.
    sourceMessages: NormalizedMessage[];
    windowEdge: MessageWindowEdge;
    isLoaded: boolean;
...
export interface SessionRowData {
    ...
    hasUnread: boolean;
    // Native project identity supplied by Rig. LMC CLI project cards derive
    // their identity from machineId + path instead.
    projectId: string | null;
    // Raw account-scoped server linkage. Unlike projectId, this never falls
    // back to encrypted Rig metadata. The record flag stays false until that
    // exact Project has been fetched and decrypted.
    serverProjectId: string | null;
    serverProjectRecordAvailable: boolean;
    ...
    projectHasCustomAvatar: boolean;
}
...
function buildSessionRowData(
    ...
    const serverProjectId = session.projectId ?? null;
    const serverProject = serverProjectId ? projects[serverProjectId] : undefined;
    const metadataProject = session.metadata?.project;
    const isRigSession = isHappyAgentSession(session);
    const projectAvatar = isRigSession ? serverProject?.avatar : null;
    return {
        id: session.id,
        ...
        projectId,
        serverProjectId,
        serverProjectRecordAvailable: serverProject?.id === serverProjectId,
        ...
        projectHasCustomAvatar: isRigSession && serverProject?.hasAvatar === true,
    };
}
...
interface StorageState {
    ...
    applyLoaded: () => void;
    applyReady: () => void;
    applyMessages: (sessionId: string, messages: NormalizedMessage[], options?: { edge?: MessageWindowEdge }) => { changed: string[], hasReadyEvent: boolean, enteredPlanMode: boolean, windowRebuilt: boolean };
    applyMessagesLoaded: (sessionId: string) => void;
    // Frees a background session's message window (web memory bound). No-op
    // for the session currently on screen. The next visit does a clean
    // initial load, so callers must also reset their fetch cursors.
    releaseSessionMessages: (sessionId: string) => void;
...
function sessionNamesFromSettings(state: StorageState, settings: Settings) {
    if (equal(state.settings.sessionNameOverrides, settings.sessionNameOverrides)) return {};
    const sessions = Object.fromEntries(Object.entries(state.sessions).map(([id, session]) => {
        const customName = settings.sessionNameOverrides?.[id] ?? null;
        return [id, session.customName === customName ? session : { ...session, customName }];
    }));
    return {
        sessions,
        sessionsData: state.sessionsData?.map(item => typeof item === 'string' ? item : sessions[item.id]) ?? null,
        sessionListViewData: buildSessionListViewData(sessions, state.unreadSessionIds, state.machines, state.projects),
    };
}
    ...
    return {
        ...
        applySessions: (sessions: (Omit<Session, 'presence'> & { presence?: "online" | number })[]) => set((state) => {
            ...
            sessions.forEach(incoming => {
                ...
                // A worker the hub started with autonomy runs at full permission by
                // derivation, not by a stored pick — so unbinding it drops the derived
                // default instead of overwriting whatever the person had chosen.
                const prior = state.sessions[session.id];
                const wasAutomaticWorker = prior?.metadata?.orchestration?.role === 'worker'
                    && prior.metadata.orchestration.hub.autonomy === true;
                const isAutomaticWorker = session.metadata?.orchestration?.role === 'worker'
                    && session.metadata.orchestration.hub.autonomy === true;
                const permissionBase = wasAutomaticWorker && !isAutomaticWorker
                    ? session.metadata?.permissionMode ?? null : resolveModePick('permissionMode');
                const resolvedPermissionMode = isAgentModePushPending(session.id, 'permissionMode')
                    ? prior?.permissionMode ?? null
                    : resolveWorkerPermissionMode({ ...session.metadata, permissionMode: permissionBase }, session.metadata?.flavor ?? '') ?? null;
                ...
                // `thinking` only ever moves forward in time — see sessionThinking.ts.
                const resolvedThinking = resolveThinking(state.sessions[session.id], session);
                if (resolvedThinking.stale) {
                    log.log(`⏮️ Storage: dropped stale thinking for ${session.id} (${session.thinkingAt} < ${resolvedThinking.thinkingAt})`);
                }

                mergedSessions[session.id] = {
                    ...session,
                    presence,
                    customName: state.settings.sessionNameOverrides?.[session.id] ?? null,
                ...
                if (existingSessionMessages && newSession.agentState &&
                    ...
                    if (processedMessages.length > 0) {
                        ...
                        updatedSessionMessages[session.id] = {
                            ...
                            sourceMessages: existingSessionMessages.sourceMessages,
                            windowEdge: existingSessionMessages.windowEdge,
        ...
        applyMessages: (sessionId: string, messages: NormalizedMessage[], options?: { edge?: MessageWindowEdge }) => {
            ...
            // A model's planning tool is not an explicit owner policy pick.
            // Bound workers keep the hub/user policy; their runtime handles
            // plan completion without turning it into another human approval.
            const binding = get().sessions[sessionId]?.metadata?.orchestration;
            if (binding?.role === 'worker' && binding.hub.autonomy === true) shouldEnterPlanMode = false;
            ...
            set((state) => {
                ...
                // Native clients keep the existing incremental reducer path.
                // Web retains complete recent turns; at a completion boundary,
                // replace the reducer and display map so the older
                // object graph becomes unreachable and can be collected.
                const isWebWindow = Platform.OS === 'web';
                // A realtime batch carrying a user prompt means the person is
                // at the composer, so the window snaps back to newest-priority.
                // Explicit edges (initial fetch, older pages) always win, and a
                // sticky 'older' edge otherwise survives streaming updates so
                // scrollback reading is not yanked away.
                const impliedEdge = messages.some((message) => message.role === 'user')
                    ? 'newest'
                    : existingSession.windowEdge;
                const requestedEdge = options?.edge ?? impliedEdge;
                const windowResult = isWebWindow
                    ? mergeAndSelectMessageWindow(existingSession.sourceMessages, messages, requestedEdge)
                    : null;
                const shouldRebuild = !!windowResult && (
                    windowResult.evictedIds.length > 0 ||
                    windowResult.compactedIds.length > 0
                );

                let processedMessages: Message[];
                let mergedMessagesMap: Record<string, Message>;
                let nextReducerState = existingSession.reducerState;
                let reducerTodos: ReturnType<typeof reducer>['todos'];
                const rebuildChangedIds: string[] = [];
                if (shouldRebuild && windowResult) {
                    hasReadyEvent = messages.some(message => message.role === 'event'
                        && message.content.type === 'ready' && !message.isSidechain
                        && !existingSession.reducerState.messageIds.has(message.id));
                    const rebuilt = rebuildDerivedMessageWindow(windowResult.messages, agentState);
                    // Rebuilding replaces every derived object; hand back the
                    // previous object wherever the content is identical so row
                    // memoization holds and the rebuild is invisible on screen.
                    mergedMessagesMap = {};
                    for (const [id, next] of Object.entries(rebuilt.messagesMap)) {
                        const previous = existingSession.messagesMap[id];
                        if (previous && derivedMessagesEqual(previous, next)) {
                            mergedMessagesMap[id] = previous;
                        } else {
                            mergedMessagesMap[id] = next;
                            rebuildChangedIds.push(id);
                        }
                    }
                    processedMessages = Object.values(mergedMessagesMap)
                        .sort(compareDisplayMessages);
                    nextReducerState = rebuilt.reducerState;
                    reducerTodos = rebuilt.todos;
                } else {
                    const incremental = reducer(existingSession.reducerState, messages, agentState);
                    processedMessages = incremental.messages;
                    reducerTodos = incremental.todos;
                    if (incremental.hasReadyEvent) {
                        hasReadyEvent = true;
                    }
                ...
                windowRebuilt = shouldRebuild;
                if (shouldRebuild) {
                    if (requestedEdge !== 'older') {
                        for (const id of rebuildChangedIds) changed.add(id);
                    }
                } else {
                    for (const message of processedMessages) changed.add(message.id);
                }
                ...
                const needsUpdate = (reducerTodos !== undefined || nextReducerState.latestUsage || shouldEnterPlanMode) && session;
                ...
                if (needsUpdate) {
                    updatedSessions = {
                        ...
                        [sessionId]: {
                            ...session,
                            ...(reducerTodos !== undefined && { todos: reducerTodos }),
                            // Copy latestUsage from reducerState to make it immediately available
                            latestUsage: nextReducerState.latestUsage ? {
                                ...nextReducerState.latestUsage
                            } : session.latestUsage,
                ...
                return {
                    ...
                    sessionMessages: {
                        ...
                        [sessionId]: {
                            ...
                            sourceMessages: windowResult?.messages ?? existingSession.sourceMessages,
                            // The server may previously have reported the beginning of
                            // history. Eviction makes that history loadable again.
                            hasMoreOlder: existingSession.hasMoreOlder || !!windowResult?.evictedIds.length,
                            windowEdge: isWebWindow ? requestedEdge : existingSession.windowEdge,
                            isLoaded: true
                        }
                    }
                };
            ...
            return { changed: Array.from(changed), hasReadyEvent, enteredPlanMode: shouldEnterPlanMode, windowRebuilt };
        },
        releaseSessionMessages: (sessionId: string) => set((state) => {
            if (state.currentViewingSessionId === sessionId) return state;
            if (!state.sessionMessages[sessionId]) return state;
            const { [sessionId]: released, ...remaining } = state.sessionMessages;
        ...
        applySettingsLocal: (settings: Partial<Settings>) => set((state) => {
            const merged = applySettings(state.settings, settings);
            saveSettings(merged, state.settingsVersion ?? 0);
            return {
                ...state,
                ...sessionNamesFromSettings(state, merged),
                settings: merged
            };
        }),
        applySettings: (settings: Settings, version: number) => set((state) => {
            if (state.settingsVersion === null || state.settingsVersion < version) {
                ...
                return {
                    ...state,
                    ...sessionNamesFromSettings(state, settings),
                    settings,
                    settingsVersion: version
                };
            } else {
...
/** Finished while you were elsewhere, and not opened since. */
export function useSessionUnread(id: string): boolean {
    return storage((state) => state.unreadSessionIds.has(id));
}
...
// Web-only memory diagnostic: per-session window sizes, largest first.
// Run `__happyMemoryStats()` in the browser console to see where the heap
// goes and to verify that background-session windows are actually released.
if (Platform.OS === 'web' && typeof globalThis !== 'undefined') {
    (globalThis as any).__happyMemoryStats = () => {
        const state = storage.getState();
        const sessions = Object.entries(state.sessionMessages).map(([sessionId, session]) => ({
            sessionId,
            title: state.sessions[sessionId] ? getSessionName(state.sessions[sessionId]).slice(0, 40) : '(untitled)',
            sourceCount: session.sourceMessages.length,
            derivedCount: session.messages.length,
            estimatedSourceBytes: session.sourceMessages.reduce(
                (total, message) => total + estimateNormalizedMessageBytes(message), 0),
            viewing: state.currentViewingSessionId === sessionId,
        })).sort((a, b) => b.estimatedSourceBytes - a.estimatedSourceBytes);
        // The pieces that are not the message window, so a single call says
        // where the heap actually is rather than only where it was expected to be.
        const reducerBytes = Object.values(state.sessionMessages).reduce((total, session) => {
            const r = session.reducerState;
            return total + estimateNormalizedMessageBytes([...r.messages.values()])
                + estimateNormalizedMessageBytes([...r.sidechains.values()])
                + estimateNormalizedMessageBytes([...r.permissions.values()]);
        }, 0);
        const agentStateBytes = Object.values(state.sessions).reduce((total, session) => total + estimateNormalizedMessageBytes(session.agentState ?? null), 0);
        const metadataBytes = Object.values(state.sessions).reduce((total, session) => total + estimateNormalizedMessageBytes(session.metadata ?? null), 0);
        const perf = (globalThis as any).performance?.memory;
        return {
            materializedSessions: sessions.length,
            totalEstimatedSourceBytes: sessions.reduce((total, row) => total + row.estimatedSourceBytes, 0),
            reducerBytes,
            sessionsInStore: Object.keys(state.sessions).length,
            agentStateBytes,
            metadataBytes,
            // Chrome only: the JS heap the renderer reports, for comparison with
            // the tab's footprint — a large gap points at images, DOM or GPU rather
            // than at anything held in JavaScript.
            jsHeapUsedMB: perf ? Math.round(perf.usedJSHeapSize / 1048576) : null,
            jsHeapLimitMB: perf ? Math.round(perf.jsHeapSizeLimit / 1048576) : null,
            sessions,
        };
    };
}
