...
export async function downloadProjectAvatar(
    ...
    const headers = isServerHostedUrl(downloadUrl, serverUrl)
        ? { Authorization: `Bearer ${credentials.token}`, 'X-Happy-Client': getLmcClientId() }
        : undefined;
...
/** Descriptor returned by the server for a granted avatar upload slot. */
export interface ProjectAvatarUpload {
    ref: string;
    uploadUrl: string;
    method: 'PUT' | 'POST';
...
/**
 * Ask for an upload slot for `size` encrypted bytes. The server owns the ref;
 * a client cannot choose where its avatar lands. Rate limited server-side, so
 * a rejected request is a 429 rather than a retryable network failure.
 */
export async function requestProjectAvatarUpload(
    credentials: AuthCredentials,
    projectId: string,
    size: number,
): Promise<ProjectAvatarUpload> {
    const endpoint = `${getServerUrl()}/v1/projects/${encodeURIComponent(projectId)}/avatar/request-upload`;
    const response = await fetch(endpoint, {
        method: 'POST',
        headers: authHeaders(credentials),
    ...
    if (!response.ok) {
        throw new Error(`Failed to request project avatar upload: ${response.status}`);
    }
    const body: unknown = await response.json();
    if (!isRecord(body)
        ...
        throw new Error('Project avatar upload response was malformed');
    }
    const formFields = isRecord(body.formFields)
        ? Object.fromEntries(
            Object.entries(body.formFields).filter((entry): entry is [string, string] => typeof entry[1] === 'string'),
        )
        : undefined;
    return {
        ref: body.ref,
        uploadUrl: rewriteLoopbackHost(body.uploadUrl),
        method: body.method,
        formFields,
    };
}
...
/**
 * Point the project at an uploaded ref. `preview` is ciphertext of the
 * {thumbhash, mimeType} JSON under the project key; the server stores it
 * opaquely and bumps avatarVersion, which fans the change out to other clients.
 */
export async function activateProjectAvatar(
    credentials: AuthCredentials,
    projectId: string,
    ref: string,
    preview: string,
): Promise<void> {
    const endpoint = `${getServerUrl()}/v1/projects/${encodeURIComponent(projectId)}/avatar`;
    const response = await fetch(endpoint, {
        method: 'PATCH',
        headers: authHeaders(credentials),
        body: JSON.stringify({ ref, preview }),
    });
    if (!response.ok) {
        throw new Error(`Failed to activate project avatar: ${response.status}`);
    }
}

/** Clear the project avatar, falling the UI back to the generated one. */
export async function clearProjectAvatar(
    credentials: AuthCredentials,
    projectId: string,
): Promise<void> {
    const endpoint = `${getServerUrl()}/v1/projects/${encodeURIComponent(projectId)}/avatar`;
    ...
    if (!response.ok && response.status !== 404) {
        throw new Error(`Failed to clear project avatar: ${response.status}`);
    }
}
