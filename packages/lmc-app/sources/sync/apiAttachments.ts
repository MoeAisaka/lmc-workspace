...
export async function requestAttachmentUpload(
    ...
    return { ...result, uploadUrl: result.method === 'PUT' ? resolveAttachmentUrl(result.uploadUrl, API_ENDPOINT, sessionId) : rewriteLoopbackHost(result.uploadUrl) };
}
...
export async function uploadEncryptedBlob(
    ...
    const isServerUrl = isSameServerUrl(upload.uploadUrl, serverUrl);
...
export async function downloadEncryptedAttachment(
    ...
    const downloadUrl = resolveAttachmentUrl(rewriteLoopbackHost(rawDownloadUrl), API_ENDPOINT, sessionId);

    const isServerUrl = isSameServerUrl(downloadUrl, API_ENDPOINT);
    const headers: Record<string, string> = {};
    if (isServerUrl) {
        headers['Authorization'] = `Bearer ${credentials.token}`;
    }
