...
export function reducer(state: ReducerState, messages: NormalizedMessage[], agentState?: AgentState | null): ReducerResult {
    ...
    for (const { message, event } of convertedEvents) {
        const mid = derivedId('evt', message.id);
    ...
    for (let msg of nonSidechainMessages) {
        if (msg.role === 'user') {
            ...
            for (let [contentIndex, c] of msg.content.entries()) {
                if (c.type === 'text' || c.type === 'thinking') {
                    let mid = derivedId('txt', `${msg.id}:${contentIndex}`);
    ...
    const sourceSequences = new Map(messages.map(message => [message.id, message.serverSeq]));
    for (let id of changed) {
        let existing = state.messages.get(id);
        if (!existing) continue;

        const sourceSequence = existing.realID ? sourceSequences.get(existing.realID) : undefined;
        if (sourceSequence !== undefined && existing.serverSeq === undefined) existing.serverSeq = sourceSequence;
...
// Derived-message ids are deterministic functions of their source, so a full
// window rebuild replays to the same ids. Random ids gave every rebuilt row a
// new React key — the whole chat unmounted and remounted on each trim, which
// is what flashed on screen. Deterministic ids also make the map-overwrite a
// natural dedupe when the same source is ever replayed twice. The `d:` prefix
// keeps them clearly apart from server-assigned message ids.
function derivedId(kind: 'evt' | 'perm' | 'usr' | 'txt' | 'tool', key: string): string {
    return `d:${kind}:${key}`;
}
...
function convertReducerMessageToMessage(reducerMsg: ReducerMessage, state: ReducerState): Message | null {
    if (reducerMsg.role === 'user' && reducerMsg.text !== null) {
        return {
            ...
            ...(reducerMsg.serverSeq !== undefined ? { serverSeq: reducerMsg.serverSeq } : {}),
            kind: 'user-text',
