import { resolveSessionLifecycle, resolveFetchedSessionLifecycle, type SessionLifecycle } from './sessionLifecycle';
import { UnsupportedCodexEffortError } from '@/components/modelModeOptions';
import Constants from 'expo-constants';
import * as Device from 'expo-device';
import { apiSocket, getCurrentAppState, getLmcClientId } from '@/sync/apiSocket';
...
import { lmcIdentity } from '@/auth/lmcAuth';
...
import { clearProjectAvatar, fetchProjects as fetchProjectRecords } from './apiProjects';
import { decryptProjectRecord, loadProjectAvatar, uploadProjectAvatar, type DecryptedProjectRecord, type ProjectAvatarUploadInput } from './projects';
import type { Project, ProjectAvatar } from './projectTypes';
import { ProjectAvatarMutationLock } from '@/utils/projectAvatarMutationLock';
...
class Sync {
    ...
    private projectAvatarMutations = new ProjectAvatarMutationLock();
    ...
    async create(credentials: AuthCredentials, encryption: Encryption) {
        ...
        this.serverID = await lmcIdentity(credentials.token);
        await this.#init();
    ...
    // Sessions whose message windows are kept fully materialized: the one on
    // screen plus the last few visited (side chat panel views two at once).
    // Everything older is released outright — window, reducer, fetch cursors —
    // so a day of hopping between sessions cannot accumulate one window per
    // session visited. A released session reloads from scratch on next visit.
    private recentlyViewedSessions: string[] = [];
    private static readonly MAX_MATERIALIZED_SESSIONS = 3;

    private noteSessionViewed(sessionId: string) {
        this.recentlyViewedSessions = [
            sessionId,
            ...this.recentlyViewedSessions.filter((id) => id !== sessionId),
        ];
        while (this.recentlyViewedSessions.length > Sync.MAX_MATERIALIZED_SESSIONS) {
            const evicted = this.recentlyViewedSessions.pop()!;
            storage.getState().releaseSessionMessages(evicted);
            // Reset fetch cursors so the next visit does a clean initial load
            // instead of a forward sync on top of a window that no longer exists.
            this.sessionLastSeq.delete(evicted);
            this.sessionOldestSeq.delete(evicted);
            this.sessionMessageQueue.delete(evicted);
        }
    }
    ...
    applySettings = (delta: Partial<Settings>) => {
        ...
        this.pendingSettings = {
            ...this.pendingSettings, ...delta,
            ...(delta.projectAvatarOverrides ? { projectAvatarOverrides: { ...this.pendingSettings.projectAvatarOverrides, ...delta.projectAvatarOverrides } } : {}),
            ...(delta.sessionProjectOrder ? { sessionProjectOrder: { ...this.pendingSettings.sessionProjectOrder, ...delta.sessionProjectOrder } } : {}),
            ...(delta.sessionNameOverrides ? { sessionNameOverrides: {
                ...this.pendingSettings.sessionNameOverrides, ...delta.sessionNameOverrides,
            } } : {}),
        };
    ...
    purchaseProduct = async (_productId: string): Promise<{ success: boolean; error?: string }> => ({success:false,error:t('lmc.common.noSubscription')});
    getOfferings = async (): Promise<{success:boolean;offerings?:any;error?:string}> => ({success:true,offerings:{current:null,all:{}}});
    presentPaywall = async (_flow?: string): Promise<{success:boolean;purchased?:boolean;error?:string}> => ({success:false,error:t('lmc.common.noSubscription')});
    ...
    /**
     * Publish a cropped image as the project avatar.
     *
     * Requires the project to be synced: its data key is what the image is
     * encrypted under, and without it the upload could only produce ciphertext
     * no client could read back. The local cache is dropped and the catalog
     * re-fetched so every surface showing the old avatar repaints.
     */
    subscribeProjectAvatarMutations = (listener: () => void): (() => void) => (
        this.projectAvatarMutations.subscribe(listener)
    );

    isProjectAvatarMutationInFlight = (projectId: string): boolean => (
        this.projectAvatarMutations.isLocked(projectId)
    );

    async setGroupAvatar(groupId: string, prepareImage: (() => Promise<ProjectAvatarUploadInput>) | null): Promise<void> {
        await this.projectAvatarMutations.run('group:' + groupId, async () => {
            if (!this.credentials) throw new Error('Not authenticated');
            const image = prepareImage ? await prepareImage() : null;
            if (image && (image.bytes.length === 0 || image.bytes.length > 100000)) throw new Error('Avatar exceeds thumbnail limit');
            this.applySettings({ projectAvatarOverrides: { [groupId]: image ? {
                uri: `data:${image.mimeType};base64,${encodeBase64(image.bytes, 'base64')}`,
    ...
    async setProjectAvatar(
        projectId: string,
        prepareImage: () => Promise<ProjectAvatarUploadInput>,
    ): Promise<void> {
        await this.projectAvatarMutations.run(projectId, async () => {
            if (!this.credentials) throw new Error('Not authenticated');
            if (!this.projectDataKeys.has(projectId)) {
                throw new Error('Project has not been synced yet');
            }
            const dataKey = this.projectDataKeys.get(projectId) ?? null;
            const blobKey = await this.encryption.getProjectBlobKey(dataKey);
            const image = await prepareImage();
            await uploadProjectAvatar(this.credentials, projectId, image, dataKey, blobKey, this.encryption);
            this.clearProjectAvatarCache(projectId);
            this.projectsSync.invalidate();
        });
    }

    /** Drop the custom avatar; the UI falls back to the generated identity. */
    async removeProjectAvatar(projectId: string): Promise<void> {
        await this.projectAvatarMutations.run(projectId, async () => {
            if (!this.credentials) throw new Error('Not authenticated');
            await clearProjectAvatar(this.credentials, projectId);
            this.clearProjectAvatarCache(projectId);
            this.projectsSync.invalidate();
        });
    }
    ...
    private prefetchOlderMessagesInBackground = async (sessionId: string) => {
        ...
        // 本地补丁（slopus/happy#1453）：后台预取只补一小段滚动缓冲，
        // 其余历史留给 loadOlderMessages 在用户上滑时按需加载，
        // 避免长会话被整段解密进内存导致 web 端内存无上限增长。
        ...
        while (true) {
            ...
            if (prefetchedPages >= MAX_PREFETCH_PAGES) {
                log.log(`💬 prefetchOlderMessagesInBackground: page budget (${MAX_PREFETCH_PAGES}) reached for ${sessionId}, leaving rest to on-demand loading`);
                return;
            }
    ...
    private applyFetchedMessages = async (
        ...
        encryption: ReturnType<Encryption['getSessionEncryption']> & {},
        messages: ApiMessage[],
        edge: 'newest' | 'older' = 'newest',
    ) => {
        if (messages.length === 0) return;
        const loadedThrough = this.sessionLastSeq.get(sessionId) ?? 0;
        ...
        for (let i = 0; i < decryptedMessages.length; i++) {
            ...
            if (normalized) {
                normalizedMessages.push({ ...normalized, serverSeq: messages[i].seq });
            }
        }
        if (normalizedMessages.length > 0) {
            this.applyMessages(sessionId, normalizedMessages, { edge });
        }
    }
    ...
    loadOlderMessages = async (sessionId: string) => {
        ...
        try {
            await lock.inLock(async () => {
                ...
                await this.applyFetchedMessages(sessionId, encryption, messages, 'older');
    ...
    private handleUpdate = async (update: unknown) => {
        ...
        if (updateData.body.t === 'new-message') {
            ...
            if (updateData.body.message) {
                ...
                if (decrypted) {
                    const normalized = normalizeRawMessage(decrypted.id, decrypted.localId, decrypted.createdAt, decrypted.content);
                    lastMessage = normalized ? { ...normalized, serverSeq: updateData.body.message.seq } : null;
                    ...
                    // A session that was never loaded in this tab stays
                    // unmaterialized: unread badges and permission prompts come
                    // from session metadata (applySessions above), so fetching
                    // the message history of every background session that
                    // speaks only grows the heap — one full window per active
                    // agent, which is where multi-gigabyte tabs came from. The
                    // messages load when the user actually opens the session.
                    const isViewingSession = storage.getState().currentViewingSessionId === updateData.body.sid;
                    if (currentLastSeq === undefined && !isViewingSession) {
                        return;
                    }
            ...
            // Ping only the session on screen. Pinging every speaking session
            // refetched messages and git status for each of them and let
            // background streams rotate through the recently-viewed LRU,
            // defeating the release of background windows.
            if (storage.getState().currentViewingSessionId === updateData.body.sid) {
    ...
    private applyMessages = (sessionId: string, messages: NormalizedMessage[], options?: { edge?: 'newest' | 'older' }) => {
        const result = storage.getState().applyMessages(sessionId, messages, options);
        const residentSources = storage.getState().sessionMessages[sessionId]?.sourceMessages;
        if (residentSources && residentSources.length > 0) {
            let minResidentSeq = Number.POSITIVE_INFINITY;
            for (const source of residentSources) {
                if (source.serverSeq !== undefined && source.serverSeq < minResidentSeq) {
                    minResidentSeq = source.serverSeq;
                }
            }
            if (Number.isFinite(minResidentSeq)) {
                this.sessionOldestSeq.set(sessionId, minResidentSeq);
            }
        }
        let m: Message[] = [];
