...
export interface SpawnSessionOptions {
    ...
    /** Start the session bound as this hub's worker, with this title (【duty】name). */
    hubSessionId?: string;
    title?: string;
     ...
     * LMC session attaches to a pre-existing on-disk Claude conversation
    ...
    /** LMC session id this fork was branched from (lineage). */
...
export async function machineSpawnNewSession(options: SpawnSessionOptions): Promise<SpawnSessionResult> {
    ...
    const { machineId, directory, approvedNewDirectoryCreation = false, token, agent, permissionMode, modelMode, effortLevel, clientRequestId, providerId, modelId, effort, happyAgentTarget, resumeClaudeSessionId, resumeCodexThreadId, parentSessionId, forkedFromMessageId, isSideChat, hubSessionId, title } = options;
    ...
    try {
        ...
        const request: SpawnRequest = agent === 'rig' && happyAgentTarget
            ...
            : { type: 'spawn-in-directory', directory, approvedNewDirectoryCreation, token, agent, permissionMode, modelMode, effortLevel, resumeClaudeSessionId, resumeCodexThreadId, parentSessionId, forkedFromMessageId, isSideChat, hubSessionId, title };
 ...
 * the new Claude session UUID. Caller then spawns a fresh LMC session
...
export async function machineResumeSession(options: ResumeSessionOptions & { model?: string; permissionMode?: string }): Promise<SpawnSessionResult> {
    ...
    try {
        const isCodex = storage.getState().sessions[sessionId]?.metadata?.flavor === 'codex';
 ...
 */
/**
 * Turn agent-to-agent mail on or off for one session.
 *
 * On by default; this turns receiving off for one session. It rides in session
 * metadata so the agent and every device see the same answer. Sending never
 * needs a switch: an agent only sends because its own user asked it to.
 */
/**
 * Read-modify-write one session's metadata from the app, retried on version
 * conflict with the server's copy. The updater sees the latest metadata each
 * attempt, so it must be a pure function of it.
 */
export async function sessionUpdateMetadata(sessionId: string, update: (metadata: Record<string, unknown>) => Record<string, unknown>): Promise<void> {
    ...
    let metadata: Record<string, unknown> = update({ ...session.metadata });
    for (let attempt = 0; attempt < 3; attempt++) {
        const result = await apiSocket.emitWithAck<{ result: 'success' | 'version-mismatch' | 'error'; version?: number; metadata?: string }>(
            'update-metadata',
            { sid: sessionId, metadata: await encryption.encryptRaw(metadata), expectedVersion: version },
        );
    ...
    throw new Error('Failed to update session metadata after repeated version conflicts');
}

export async function sessionSetAgentMail(sessionId: string, enabled: boolean): Promise<void> {
    await sessionUpdateMetadata(sessionId, (metadata) => ({ ...metadata, agentMail: enabled }));
}
...
async function sessionUpdateAgentModesMetadata(
    ...
    const withPermissionSource = (metadata: Record<string, unknown>) => ({
        ...metadata, ...pendingPatch,
        ...(pendingPatch.permissionMode !== undefined ? { permissionModeSource: 'explicit' } : {}),
    });
    let currentMetadata: Record<string, unknown> = withPermissionSource(session.metadata);
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        ...
        if (result.result === 'version-mismatch') {
            ...
            currentMetadata = withPermissionSource(latest);
            continue;
        }
...
export function sessionSetAgentModes(sessionId: string, patch: SessionAgentModesPatch): void {
    ...
    if (patch.permissionMode !== undefined && (isChanged(patch.permissionMode, 'permissionMode') || session?.metadata?.permissionModeSource !== 'explicit')) {
 ...
 *   2. Spawn a new LMC session on the same machine with
 ...
 * RPC into env vars, then into the new LMC session's metadata at start
