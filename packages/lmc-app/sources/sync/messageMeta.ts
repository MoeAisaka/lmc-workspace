...
export function resolveMessageModeMeta(
    ...
    if (flavor === 'codex') {
        ...
        // An engine switch leaves the departed engine's model in session state,
        // and the CLI passes an unrecognised name straight through to the
        // app-server, which rejects the turn. Fall back to this engine's own
        // default instead — and let the effort fall back with it, since the one
        // on file was chosen for a model that is no longer running.
        const picked = session.modelMode ?? defaults.modelMode;
        const foreign = isForeignModelKey(picked, flavor);
    ...
    // Same cross-engine guard as the Codex branch above. Here the fallback may
    // itself be undefined, which is the right answer: send no model at all and
    // let the running engine resolve its own.
    const pickedModel = session.modelMode ?? agentOverrides.modelMode;
    const foreignModel = isForeignModelKey(pickedModel, flavor);
    const modelMode = foreignModel ? agentOverrides.modelMode : pickedModel;
    if (modelMode !== undefined && !isForeignModelKey(modelMode, flavor)) {
        meta.model = modelMode === 'default' ? null : modelMode;
    }
