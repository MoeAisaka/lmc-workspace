...
function estimateValueBytes(value: unknown): number {
    ...
    if (Array.isArray(value)) return 8 + value.reduce((total, item) => total + estimateValueBytes(item), 0);
    if (typeof value === 'object') return 16 + Object.values(value).reduce((total, item) => total + estimateValueBytes(item), 0);
    return 0;
}
...
export class EncryptionCache {
    ...
    constructor(options: EncryptionCacheOptions = {}) {
        this.maxMessages = options.maxMessages ?? 160;
        this.maxMessageBytes = options.maxMessageBytes ?? 8 * 1024 * 1024;
        this.maxSingleMessageBytes = options.maxSingleMessageBytes ?? 512 * 1024;
    }
    ...
    setCachedMessage(messageId: string, data: DecryptedMessage): void {
        const estimatedBytes = estimateValueBytes(data);
        const existing = this.messageCache.get(messageId);
        if (existing) {
            this.messageCacheBytes -= existing.estimatedBytes ?? 0;
            this.messageCache.delete(messageId);
        }
        // Oversized decrypted payloads are used by the caller once and then
        // released. Keeping them here would bypass the bounded UI window.
        if (estimatedBytes > this.maxSingleMessageBytes) {
            return;
        }
        ...
        this.messageCacheBytes += estimatedBytes;

        while (this.messageCache.size > this.maxMessages || this.messageCacheBytes > this.maxMessageBytes) {
            ...
            this.messageCacheBytes -= evicted?.estimatedBytes ?? 0;
    ...
    getStats() {
        return {
            ...
            messageBytes: this.messageCacheBytes,
