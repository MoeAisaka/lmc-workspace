 ...
 * LMC logs instead of giving generic networking advice.
 */
export function buildOfflineMachineTroubleshooting(
    ...
    const projectName = session?.metadata?.project?.name?.trim()
        ...
        || 'LMC';
    const happyHomeDir = choice?.lmcMachine?.metadata?.happyHomeDir?.trim()
        || choice?.rigMachine?.metadata?.happyHomeDir?.trim()
        || session?.metadata?.happyHomeDir?.trim()
        || '~/.lmc/agent';
    const aiPrompt = `In ${happyHomeDir}, diagnose why LMC cannot reach "${machineName}" for project "${projectName}".`;
