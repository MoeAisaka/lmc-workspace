...
vi.mock('@/components/modelModeOptions', async (importOriginal) => ({
    ...await importOriginal<typeof import('@/components/modelModeOptions')>(),
...
describe('useStartSessionFromDraft', () => {
    ...
    it('refuses invalid Astra draft before spawn and preserves the prompt and images', async () => {
        mocks.defaultOverrides = { codex: { modelMode: 'gpt-6-astra' } };
        mocks.draft = createDraft({ modelMode: 'gpt-6-astra', effortLevel: 'minimal' });
        ...
        expect(mocks.alert).toHaveBeenCalledWith(expect.any(String), expect.stringContaining('localFeatures.invalidEffort'));
        expect(mocks.draft.setInput).not.toHaveBeenCalled();
        expect(mocks.draft.setAttachments).not.toHaveBeenCalled();
    });

    it.each(['low', 'medium', 'high', 'xhigh', 'max', 'ultra'])('keeps explicit Astra %s when spawning from the draft', async (effort) => {
        mocks.defaultOverrides = { codex: { modelMode: 'gpt-6-astra' } };
        mocks.draft = createDraft({ modelMode: 'gpt-6-astra', effortLevel: effort });
        await expect(useStartSessionFromDraft().startSession()).resolves.toBe(true);
        expect(mocks.machineSpawnNewSession).toHaveBeenCalledWith(expect.objectContaining({ modelMode: 'gpt-6-astra', effortLevel: effort }));
    });
