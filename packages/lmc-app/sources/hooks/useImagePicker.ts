...
import { t } from '@/text';
import { attachmentSizeLimit, MAX_ATTACHMENT_FILE_SIZE, MAX_FILE_SIZE, MAX_IMAGES_PER_MESSAGE, type AttachmentPreview } from '@/sync/attachmentTypes';

export { attachmentSizeLimit, MAX_ATTACHMENT_FILE_SIZE, MAX_FILE_SIZE, MAX_IMAGES_PER_MESSAGE };
...
export function useImagePicker(): UseImagePickerResult {
    ...
    /**
     * Any file, not just images. Files are uploaded through the same encrypted
     * attachment channel and land on the session's Mac as a local path; images
     * picked this way still show as thumbnails and reach the model as images.
     */
    const pickFiles = useCallback(async () => {
        const remaining = MAX_IMAGES_PER_MESSAGE - selectedCountRef.current;
        if (remaining <= 0) {
            Modal.alert(t('imageUpload.limitTitle'), t('imageUpload.limitMessage', { max: MAX_IMAGES_PER_MESSAGE }), [{ text: t('common.ok') }]);
            return;
        }
        ...
        for (const asset of result.assets.slice(0, remaining)) {
            ...
            const pickedType = asset.mimeType || 'application/octet-stream';
            if (size > attachmentSizeLimit(pickedType)) {
                Modal.alert(t('lmc.common.fileTooLarge'), t('lmc.common.fileTooLargeBody', { name: asset.name }), [{ text: t('common.ok') }]);
                continue;
            }
            const mimeType = pickedType;
            previews.push({
                id: `${Date.now()}_${Math.random().toString(36).slice(2)}`,
                uri: asset.uri,
                width: 0,
        ...
        if (previews.length > 0) setSelectedImages(prev => [...prev, ...previews].slice(0, MAX_IMAGES_PER_MESSAGE));
    }, []);
