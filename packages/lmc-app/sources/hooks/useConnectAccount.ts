...
import { t } from '@/text';
import { accountLinkPublicKey, isAccountLinkUrl } from '@/utils/accountLinkUrl';
...
export function useConnectAccount(options?: UseConnectAccountOptions) {
    ...
    const processAuthUrl = React.useCallback(async (url: string) => {
        const linkedKey = accountLinkPublicKey(url);
        if (linkedKey === null) {
        ...
        setIsLoading(true);
        try {
            const publicKey = decodeBase64(linkedKey, 'base64url');
