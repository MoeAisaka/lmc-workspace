...
export function EmptyMainScreen({
    ...
    const enterUrlManually = React.useCallback(async () => {
        const url = await Modal.prompt(
            ...
            {
                placeholder: t('lmc.empty.pairingTitle'),
    ...
    if (machineChoices.length > 0) {
        if (hasOnlineMachines) {
            return (
                <View style={styles.container}>
                    ...
                    <Text style={styles.stateTitle}>{t('lmc.empty.noSessions')}</Text>
                    <Text style={styles.stateDescription}>{t('lmc.empty.noSessionsHint')}</Text>
                    <RoundButton title={t('lmc.empty.newSession')} size="large" onPress={() => router.navigate('/new')} />
                    {showArchivedAction}
                </View>
            );
        }
        ...
        const title = machineChoices.length === 1
            ? t('lmc.empty.unreachable', { name: machineChoices[0].name })
            : t('lmc.empty.noDevices');
        return (
            <View style={styles.container}>
                <Ionicons name="cloud-offline-outline" size={56} color={theme.colors.textSecondary} style={styles.stateIcon} />
                <Text style={styles.stateTitle}>{title}</Text>
                <Text style={styles.stateDescription}>{t('lmc.empty.deviceHint')}</Text>
                <RoundButton title={t('lmc.empty.checkConnection')} size="large" onPress={troubleshoot} />
                {showArchivedAction}
            </View>
        );
    }
                ...
                <>
                    ...
                    <View style={styles.buttonsContainer}>
                        <View style={styles.buttonWrapper}>
                            <RoundButton
                                title={t('lmc.empty.connectDevice')}
                                size="large"
                                loading={isLoading}
