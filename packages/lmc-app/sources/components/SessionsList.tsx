...
import { View, Pressable, TextInput, FlatList, NativeScrollEvent, NativeSyntheticEvent, Platform } from 'react-native';
...
export function SessionsList({
    ...
    const { theme: searchTheme } = useUnistyles();
    ...
    const data = React.useMemo<SessionListDisplayItem[] | null>(() => {
        ...
        if (query) {
            const rows = [...buildFlatSessionRows(sourceData, { sortByActivity: true }), ...sourceData.filter(item => item.type === 'session').map(item => toFlatSessionRow(item.session))];
            const seen = new Set<string>();
            return rows.filter(row => {
                if (seen.has(row.session.id)) return false;
                seen.add(row.session.id);
                return [row.session.name, row.session.subtitle, row.projectName, row.workspaceName, row.session.modelName].filter(Boolean).join(' ').toLocaleLowerCase().includes(query);
            }).map((row, index, rows) => ({type: 'flat-session' as const, row, last: index === rows.length - 1}));
        }
    ...
    }, [flatSessionList, hasArchivedSessions, hideArchivedSessions, machines, sourceData, query]);
    ...
    }, [selectedSessionId, data, flatSessionList, query]);
    ...
    return (
        <View style={[styles.container, flatSessionList && styles.containerFlat]}>
            <View style={styles.contentContainer}>
                <View style={{ marginHorizontal: 12, marginTop: topContentInset + 8, marginBottom: 8, paddingHorizontal: 10, borderRadius: 10, flexDirection: 'row', alignItems: 'center', backgroundColor: styles.container.backgroundColor, borderWidth: 1, borderColor: '#b8bec633' }}>
                    <Ionicons name="search-outline" size={17} color="#88909b" />
                    <TextInput accessibilityLabel="搜索会话" placeholder="搜索会话、项目…" placeholderTextColor="#88909b" value={searchQuery} onChangeText={setSearchQuery} style={{ flex: 1, minWidth: 0, height: 40, fontSize: 13, paddingHorizontal: 8, color: searchTheme.colors.text }} />
                    {!!searchQuery && <Pressable accessibilityRole="button" accessibilityLabel="清空搜索" onPress={() => setSearchQuery('')} hitSlop={8}><Ionicons name="close" size={17} color="#88909b" /></Pressable>}
                </View>
                {query && data.length === 0 && <Text style={{ padding: 16, color: '#88909b' }}>没有匹配的会话</Text>}
                <FlatList
                    data={data}
...
const STATUS_CONFIG: Record<SessionState, { color: string; dotColor: string; isPulsing: boolean; isConnected: boolean }> = {
    ...
    thinking: { color: '#0060F0', dotColor: '#0060F0', isPulsing: true, isConnected: true },
...
const SessionItem = React.memo(({ session, selected, isFirst, isLast, isSingle }: {
    ...
    const { anchor: actionsAnchor, closeMenu: closeActionsMenu, menuProps } = useSessionRowMenu(session.id);
    ...
    const status = session.hasUnread && !needsUserAction
        ? { ...baseStatus, color: '#0060F0', dotColor: '#0060F0', isPulsing: false, isConnected: baseStatus.isConnected }
        : baseStatus;
            ...
            <View style={styles.sessionContent}>
                ...
                <View style={styles.statusRow}>
                    <View style={styles.statusDotContainer}>
                        <SessionStatusIndicator state={session.state} showLabel={false} unread={session.hasUnread}/>
                    </View>
                    <Text style={[
                        styles.statusText,
                        { color: status.color }
