...
export const ToolView = React.memo<ToolViewProps>((props) => {
    ...
    const handlePress = React.useCallback(() => {
        if (onPress) {
            ...
            // Open in place over the transcript; a route push is the fallback
            // wherever no session view is mounted to host the overlay.
            if (!openToolDetail(messageId)) router.push(`/session/${sessionId}/message/${messageId}`);
        }
...
const styles = StyleSheet.create((theme) => ({
    ...
    compactHeader: {
        ...
        alignItems: 'center',
        minHeight: 28,
        // No card behind this row, so it starts on the same line as the prose
        // above it rather than a step inside it.
        paddingHorizontal: 0,
        paddingVertical: 3,
        borderRadius: 4,
        backgroundColor: 'transparent',
