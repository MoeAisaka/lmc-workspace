...
export const HomeDock = React.memo(({
    ...
    React.useEffect(() => {
        ...
        // Only LMC CLI's daemon answers the worktree RPC, so it is asked directly rather than
        // through whichever machine the draft happens to name.
        const lmcMachine = selectedChoice?.lmcMachine ?? null;
        if (!supportsWorktree || !lmcMachine || !isMachineOnline(lmcMachine) || !path) {
            setExistingWorktrees([]);
            return;
        }

        let cancelled = false;
        listWorktrees(lmcMachine.id, path).then((worktrees) => {
            if (cancelled) return;
    ...
    // LMC CLI's Git RPC remains only for the ordinary code-agent worktree flow.
    ...
    const worktreeOptions = React.useMemo<ModeOption[]>(() => {
        ...
        const options: ModeOption[] = [
            ...(canCreateWorktree
                ? [{ key: '__new__', name: picksWorkspaces ? t('localFeatures.createNew') : 'Create new worktree' }]
                : []),
            // Starting in no workspace means starting in the project's own
            // checkout, which is a place with a name rather than an absence.
            { key: '__none__', name: picksWorkspaces ? t('localFeatures.mainWorkspace') : 'No worktree' },
            ...existingWorktrees,
        ];
        if (
            worktreeKey
    ...
    const environmentRows: SettingsRow[] = [
        ...
        { page: 'project', label: 'PROJECT', value: currentProject?.name ?? '~', icon: 'folder-outline' },
        {
            ...
            value: currentWorktree?.name ?? (picksWorkspaces ? t('localFeatures.mainWorkspace') : 'No worktree'),
    ...
    const getEnvironmentPickerConfig = (setting: EnvironmentSetting): PickerConfig => {
        if (setting === 'machine') {
            return { title: t('localFeatures.machines'), options: machineOptions, selectedKey: selectedMachineId, onSelect: setMachineId };
        }
        if (setting === 'project') {
            return {
                title: t('localFeatures.project'),
                options: [
                    ...projectOptions,
                    {
        ...
        return {
            title: picksWorkspaces ? t('localFeatures.workspace') : t('localFeatures.worktree'),
