...
interface ChatHeaderViewProps {
    ...
    onTitlePress?: () => void;
    onBackPress?: () => void;
    /** Phone: opens the floating session drawer in place of a back arrow. */
    onMenuPress?: () => void;
    /** Second line under the title: session status in its tone colour, then device · project. */
...
export const ChatHeaderView: React.FC<ChatHeaderViewProps> = ({
    ...
    if (Platform.OS === 'web') {
        return (
            // Transparent: the transcript runs full height beneath, and only the
            // title and the actions float above it as pills.
            <View style={[styles.container, { paddingTop: insets.top }]} pointerEvents="box-none">
                {/* The transcript scrolls under a transparent header, so it
                    needs a veil of its own: a blur that fades out downward,
                    rather than a hard bar that would cut the page in two. */}
                {!isTablet && (
                    <View
                        pointerEvents="none"
                        style={[styles.webScrim, { height: insets.top + headerHeight + 20 }]}
                    />
                )}
                <View style={styles.contentWrapper}>
                    <View style={[styles.webContent, { height: isTablet ? DESKTOP_HEADER_HEIGHT : headerHeight }]}>
                        {onMenuPress && !isTablet ? (
                            <Pressable onPress={onMenuPress} hitSlop={15} accessibilityRole="button" accessibilityLabel={t('lmc.common.sessionList')} style={styles.webBackButton}>
                        ...
                        {title ? (
                            // Option A: the title reads as the head of the
                            // transcript, not as chrome floating over it.
                            <Pressable
                                style={({ pressed }) => [styles.webTitleColumn, pressed && onTitlePress ? { opacity: 0.6 } : null]}
                            ...
                            >
                                <Text
                                    numberOfLines={1}
                                    ellipsizeMode="tail"
                                    style={[styles.webTitle, { color: theme.colors.header.tint, fontSize: 16, ...Typography.default('semiBold') }]}
                                >
                                    {hasExtra ? `${title} / ${extraPathSegment}` : title}
                                </Text>
                            </Pressable>
                        ) : null}
...
const styles = StyleSheet.create((theme) => ({
    ...
    webScrim: {
        ...
        ...Platform.select({
            web: {
                ...
                backgroundColor: theme.dark ? 'rgba(0, 0, 0, 0.42)' : 'rgba(255, 255, 255, 0.5)',
                // Both the blur and the tint fade out together, so nothing
                // reads as an edge where the header stops.
    ...
    webContent: {
        flexDirection: 'row',
        // Top-aligned while the title carried a second status line; with that
        // gone it left the title riding high above the menu and action glyphs.
        alignItems: 'center',
        gap: 8,
