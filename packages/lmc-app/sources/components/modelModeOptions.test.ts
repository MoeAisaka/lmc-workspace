import { localFeatureEnglish } from '@/text/localFeatures';
import { preserveCodexEffortSelection } from './modelModeOptions';
...
describe('modelModeOptions', () => {
    ...
    it('refuses to offer a claude model left on a codex session by an engine switch', () => {
        // The residue used to be promoted to a custom-model row whose name is
        // the raw key, so the picker read `claude-fable-5-1` under a Codex icon.
        const models = getCodexModelModes();
        expect(includeConfiguredModel('codex', models, 'claude-fable-5-1')).toBe(models);
    });
...
describe('preserveCodexEffortSelection', () => {
    it('shows the explicit effort instead of hiding an invalid selection behind a fallback', () => {
