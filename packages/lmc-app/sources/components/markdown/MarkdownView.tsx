...
import { t } from '@/text';
import { isHttpMarkdownLink, parseFileReference } from './linkUtils';
...
export const MarkdownView = React.memo((props: { 
    ...
    const handleLinkPress = React.useCallback((url: string) => {
        ...
        // The same sheet the resource window opens: which device receives the
        ...
        else void openFileReference(undefined, reference.path);
    }, [props.sessionId]);
            ...
            if (span.url) {
                ...
                return (
                    <Text
                        ...
                        style={[props.baseStyle, followable && style.link, span.styles.map(s => style[s])]}
                        {...(followable && Platform.OS === 'web' ? { onClick: () => props.onLinkPress(span.url!) } as any : {})}
                        onPress={followable && Platform.OS !== 'web'
...
// CJK, kana and full-width forms occupy about two Latin character cells.
// Counting them as one is what let a Chinese column render narrower than its
// own text.
const WIDE_CHAR = /[ᄀ-ᅟ⺀-꓏가-힣豈-﫿︰-﹏＀-｠￠-￦]/;
// Inline code renders in the mono face, which is wider per character than the
// proportional default TABLE_CHAR_WIDTH was calibrated against. A path in
// backticks is the common case and the one that overflowed.
const CODE_CHAR_RATIO = 1.15;

// Rendered width of a span array, in Latin character cells — used to estimate
// column widths. Iterating the string yields code points, so surrogate pairs
// count once rather than twice.
function spansLength(spans: MarkdownSpan[]): number {
    let n = 0;
    for (const s of spans) {
        let cells = 0;
        for (const ch of s.text) cells += WIDE_CHAR.test(ch) ? 2 : 1;
        n += s.styles.includes('code') ? cells * CODE_CHAR_RATIO : cells;
    }
    return n;
}
...
const style = StyleSheet.create((theme) => ({
    ...
    tableCell: {
        ...
        // The width above is an estimate. When it falls short, clip rather than
        // let the text run across the column border into its neighbour.
        overflow: 'hidden',
    },
    ...
    tableHeaderText: {
        // A path or URL has no spaces to wrap at; without this the whole
        // token is treated as one unbreakable word and overflows the cell.
