...
import { DEFAULT_AVATAR_STYLE } from '@/utils/avatarStyle';
...
export const Avatar = React.memo((props: AvatarProps) => {
    ...
    // One generated style, no preference. Account pictures are real images with
    // an initial as the fallback, and the redesigned session list has no row
    // avatars — so the style and monochrome switches steered almost nothing.
    // Callers still pass `monochrome` explicitly for offline rows.
    const avatarStyle = DEFAULT_AVATAR_STYLE;
    const { theme } = useUnistyles();
    // The badge is the caller's choice now. The two settings that used to gate
    // it only reached the rows and the session-header avatar the redesign
    // retired, so they were a switch over nothing.
    const showHarnessIcon = badgeLocation !== 'none';
