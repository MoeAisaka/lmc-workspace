...
// permissionModeLabels.ts. They are LMC's own vocabulary, not a quote of each
...
export function getClaudePermissionModes(translate: Translate): PermissionMode[] {
    return [
        { key: 'auto', name: translate('localFeatures.auto'), description: translate('agentInput.permissionMode.auto'), sinceCliVersion: CLI_VERSION_WITH_AUTO },
        { key: 'acceptEdits', name: translate('localFeatures.edits'), description: translate('agentInput.permissionMode.acceptEdits') },
        { key: 'plan', name: translate('localFeatures.plan'), description: translate('agentInput.permissionMode.plan') },
        { key: 'bypassPermissions', name: translate('localFeatures.yolo'), description: translate('agentInput.permissionMode.bypassPermissions') },
        { key: 'default', name: translate('localFeatures.defaultLabel'), description: translate('agentInput.permissionMode.default') },
    ];
}
...
// more. `default` is LMC's stricter baseline — `untrusted` + workspace-write,
...
export function getCodexPermissionModes(translate: Translate): PermissionMode[] {
    return [
        { key: 'auto', name: translate('localFeatures.auto'), description: translate('agentInput.codexPermissionMode.autoDescription'), sinceCliVersion: CLI_VERSION_WITH_AUTO },
        { key: 'safe-yolo', name: translate('localFeatures.workspace'), description: translate('agentInput.codexPermissionMode.safeYoloDescription') },
        { key: 'read-only', name: translate('localFeatures.readOnly'), description: translate('agentInput.codexPermissionMode.readOnlyDescription') },
        { key: 'yolo', name: translate('localFeatures.yolo'), description: translate('agentInput.codexPermissionMode.yoloDescription') },
        { key: 'default', name: translate('localFeatures.defaultLabel'), description: translate('agentInput.codexPermissionMode.defaultDescription') },
    ];
}
...
export function getGeminiPermissionModes(translate: Translate): PermissionMode[] {
    return [
        { key: 'yolo', name: translate('localFeatures.yolo'), description: translate('agentInput.geminiPermissionMode.yolo') },
        { key: 'default', name: translate('localFeatures.defaultLabel'), description: translate('agentInput.geminiPermissionMode.default') },
    ];
}
...
/**
 * The engine whose own catalog claims this model key, or null when none does.
 *
 * A custom key nobody lists is not foreign, it is unknown — those belong to
 * whoever configured them and must keep working. This only recognises a key
 * that demonstrably names the *other* engine's model, which is what an engine
 * switch leaves behind in session state.
 */
export function engineOwningModelKey(key: string | null | undefined): 'claude' | 'codex' | null {
    if (!key || key === 'default') return null;
    if (getClaudeModelModes().some((model) => model.key === key)) return 'claude';
    if (getCodexModelModes().some((model) => model.key === key)) return 'codex';
    return null;
}

/** True when the key names a model belonging to an engine other than this one. */
export function isForeignModelKey(key: string | null | undefined, flavor: string | null | undefined): boolean {
    const owner = engineOwningModelKey(key);
    return owner !== null && owner !== flavor;
}
...
export function includeConfiguredModel(
    ...
    if (
        ...
        // A key the other engine owns is residue from a switch, not a model this
        // session can run. Promoting it to a "custom model" row renders the old
        // engine's name as though the picker had chosen it.
        || isForeignModelKey(configuredModelKey, flavor)
    ) {
        return models;
    }
    return [
        ...models,
        {
            ...
            description: translate ? translate('localFeatures.customModelDescription') : 'custom model',
        },
    ];
}
...
export function getOpenClawPermissionModes(translate: Translate): PermissionMode[] {
    return [
        { key: 'bypassPermissions', name: translate('localFeatures.yolo'), description: translate('agentInput.permissionMode.openclawInert') },
        { key: 'default', name: translate('localFeatures.defaultLabel'), description: translate('agentInput.permissionMode.openclawInert') },
    ];
}
...
export function getAgyPermissionModes(translate: Translate): PermissionMode[] {
    return [
        { key: 'default', name: translate('localFeatures.defaultLabel'), description: translate('agentInput.permissionMode.agyDefault') },
        { key: 'bypassPermissions', name: translate('localFeatures.yolo'), description: translate('agentInput.permissionMode.bypassPermissions') },
    ];
}
...
/**
 * A level's name is the word the engine itself uses — `xHigh`, not a
 * translation of it.
 *
 * These are the same kind of vocabulary as the permission modes: the name is
 * an identifier you will also see in the CLI, in a config file and in the
 * engine's own docs, and a translated one cannot be looked up anywhere. It used
 * to render as "更高（xHigh）", which spent a third of the composer chip saying
...
export class UnsupportedCodexEffortError extends Error {
    constructor(readonly model: string, readonly effort: string, readonly levels: readonly string[]) {
    ...
    localizedMessage(translate: Translate): string {
        return translate('localFeatures.invalidEffort') + this.levels.map(effortDisplayName).join('、') + ` (${this.model}: ${this.effort})`;
    }
}
...
// correct it. Resolving it to another option would silently change the request.
export function preserveCodexEffortSelection(
    flavor: AgentFlavor,
