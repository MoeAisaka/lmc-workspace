...
import { t } from '@/text';
import { ProjectGroupData, ProjectWorkspaceGroup, useSessionGitStatus, useSetting } from '@/sync/storage';
...
import { editableProjectAvatarId } from '@/utils/projectAvatarEligibility';

// Tall enough to span the name and branch lines together.
const HEADER_AVATAR_SIZE = Platform.OS === 'web' ? 20 : 30;
 ...
 * worktrees reported by Rig or created through LMC. Each worktree gets its
 ...
 */
export const ProjectGroup = React.memo(({ project, selectedSessionId, sortingDisabled }: ProjectGroupProps) => {
...
const WorkspaceSection = React.memo(({ project, workspace, selectedSessionId, sortingDisabled }: {
    project: ProjectGroupData;
    ...
    const editableAvatarProjectId = firstSession
        ? editableProjectAvatarId({
            groupId: project.id,
            serverProjectId: firstSession.serverProjectId,
            clientId: firstSession.clientId,
            projectRecordAvailable: firstSession.serverProjectRecordAvailable,
            platform: Platform.OS,
        })
        : null;
    const avatarOverride = useSetting('projectAvatarOverrides')[project.id];
    const avatarUri = editableAvatarProjectId ? firstSession?.projectAvatarUri : avatarOverride?.uri;
    const avatarThumbhash = editableAvatarProjectId ? firstSession?.projectAvatarThumbhash : avatarOverride?.thumbhash;
    ...
    return (
        <View style={styles.section}>
            <View style={styles.header}>
                <ProjectAvatarButton projectId={editableAvatarProjectId ?? project.id}
                    ordinary={!editableAvatarProjectId} projectName={project.name}
                    hasCustomAvatar={editableAvatarProjectId ? !!firstSession?.projectHasCustomAvatar : !!avatarOverride}>
                {firstSession && <Avatar id={firstSession.avatarId} size={HEADER_AVATAR_SIZE} flavor={null} imageUrl={avatarUri} thumbhash={avatarThumbhash} />}
                <View style={styles.headerText}>
                    <Text style={styles.title} numberOfLines={1}>
                        {project.name}
                    </Text>
                    {(Platform.OS !== 'web' || branchName !== 'main' || changes) && <View style={styles.branchLine}>
            ...
            <View style={styles.workspaceCard}>
                <SortableSessionRows disabled={sortingDisabled} groupId={JSON.stringify([project.id, workspace.id])} sessions={workspace.sessions} renderRow={(session, index) => (
                    <CompactSessionRow
                        key={session.id}
                        session={session}
...
const stylesheet = StyleSheet.create((theme) => ({
    ...
    title: {
        ...
        fontSize: Platform.select({ web: 12, ios: 13, default: 14 }),
    ...
    addButton: {
        ...
        backgroundColor: Platform.OS === 'web' ? 'transparent' : theme.colors.surfaceHighest,
        alignItems: 'center',
    ...
    workspaceCard: {
        ...
        shadowOpacity: Platform.select({ web: 0, default: 0 }),
        shadowRadius: 0,
        elevation: Platform.select({ web: 0, default: 0 }),
    },
}));
