...
import { openSessionFile, openToolDetail } from '@/components/lmc/toolOverlayStore';
...
export const ToolGroupView = React.memo<ToolGroupViewProps>((props) => {
    ...
    const handleSingleToolPress = React.useCallback(() => {
        ...
        const filePath = isFileEditTool(singleToolMessage.tool.name) && typeof singleToolMessage.tool.input?.file_path === 'string'
            ...
            : null;
        if (filePath) {
            if (!openSessionFile(filePath)) router.push(`/session/${sessionId}/file?path=${btoa(filePath)}`);
            return;
        }
        if (!openToolDetail(singleToolMessage.id)) router.push(`/session/${sessionId}/message/${singleToolMessage.id}`);
    ...
    const body = (
        <View style={nested ? styles.nestedInnerContainer : [styles.innerContainer, { maxWidth: chatMaxWidth }]}>
            <CollapseHeader
                expanded={expanded}
...
function ToolSummaryRow(props: {
    ...
    const handlePress = React.useCallback(() => {
        if (filePath) {
            if (!openSessionFile(filePath)) router.push(`/session/${props.sessionId}/file?path=${btoa(filePath)}`);
            return;
        }
        if (!openToolDetail(props.message.id)) router.push(`/session/${props.sessionId}/message/${props.message.id}`);
