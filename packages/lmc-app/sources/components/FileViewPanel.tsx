...
import { decodeBase64ToBytes, isBinaryExtension, looksBinary } from '@/utils/lmc/fileKinds';
    ...
    React.useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (looksBinary(rawBytes, decodedContent)) {
                    setFileState({ kind: 'binary' });
                    return;
                }
        ...
        <View style={styles.outer}>
            ...
            {showConflictDiff && externalChange && fileState.kind === 'loaded' ? (
                ...
                <View style={styles.centered}>
                    ...
                    {/* Not a dead end: the browser can hold what the viewer cannot render. */}
                    <Pressable
                        ...
                        onPress={async () => {
                            ...
                            try {
                                ...
                                if (!response.success || !response.content) throw new Error(response.error || t('files.failedToRead'));
                                await deliverResourceFile({ name, content: response.content }, 'download');
                    ...
                    >
                        <Text style={{ color: theme.colors.surface, fontSize: 13, ...Typography.default('semiBold') }}>
                            {t('lmc.files.download')}
                        </Text>
                    </Pressable>
                </View>
