...
export const CompactSessionRow = React.memo(({ session, selected, showBorder }: { session: SessionRowData; selected?: boolean; showBorder?: boolean }) => {
    ...
    const [archivingSession, performArchive] = useLmcAction(async () => {
        ...
        if (!result.success) {
            throw new LmcError(result.message || t('sessionInfo.failedToArchiveSession'), false);
        }
    });
    ...
    const renderTrailingIndicator = () => {
        return <View style={{minWidth:74,alignItems:'flex-end',justifyContent:'center'}}>
            <SessionStatusIndicator state={session.state} unread={session.hasUnread} showLabel={Platform.OS !== 'web'} />
            {session.state === 'waiting' && session.hasDraft && <Ionicons name="create-outline" size={12} color={theme.colors.textSecondary}/>}
        </View>;
    };
                ...
                <View style={styles.sessionTitleRow}>
                    <Text
                        style={[
                            ...
                            selected && Platform.OS === 'web' && { color: '#0060F0' }
                        ]}
...
const stylesheet = StyleSheet.create((theme) => ({
    ...
    sessionRowSelected: {
        backgroundColor: Platform.OS === 'web' ? '#0060F014' : theme.colors.surfaceSelected,
    },
    sessionContent: {
        flex: 1,
        justifyContent: 'center',
