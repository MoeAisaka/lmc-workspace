...
describe('usage row presence', () => {
    const presence = (hasFigures: boolean, hasEverShown: boolean) =>
        agentInputLayout.resolveUsageRowPresence({ hasFigures, hasEverShown });

    it('takes no space in a session that has never reported usage', () => {
        expect(presence(false, false)).toBe('absent');
    });

    it('keeps its place when the figures go missing, so the composer stays put', () => {
