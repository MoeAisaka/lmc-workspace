...
export const FlatSessionRow = React.memo(({ row, selected, showBorder, archived }: {
    ...
    const [archiving, performArchive] = useLmcAction(async () => {
    ...
    const content = (
        <Pressable
            style={[styles.row, selected && styles.rowSelected, SESSION_ROW_MENU_STYLE]}
            onPress={handlePress}
            {...menuProps}
        >
            ...
            <View style={[styles.content, faded && styles.contentFaded]}>
                <View style={styles.titleRow}>
                    ...
                    >
                        {session.state === 'thinking' || session.state === 'permission_required' || session.state === 'input_required' ? <SessionStatusIndicator state={session.state} unread={session.hasUnread}/> : presentation.topRight.type === 'dot' ? (
                            <StatusDot
