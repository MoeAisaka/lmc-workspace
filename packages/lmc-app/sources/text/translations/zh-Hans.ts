...
export const zhHans: TranslationStructure = {
    ...
    lmc: {
        ...
        resources: {
            searchPlaceholder: '搜索文件名、路径或正文',
            clearSearch: '清空搜索',
            fileFormat: '文件格式',
            allFormats: '全部格式',
            noExtension: '无扩展名',
            clearFilters: '清空筛选',
            searchItemCount: ({ count }: { count: number }) => `${count} 个文件`,
            searchResultCount: ({ count, total }: { count: number; total: number }) => `${count} / ${total} 项`,
            searchingBody: '正在检索文件正文…',
            searchProgress: ({ scanned, total }: { scanned: number; total: number }) => `已检索 ${scanned} / ${total} 个文件`,
            searchComplete: ({ count }: { count: number }) => `正文检索完成 · ${count} 个文件`,
            searchFailed: '正文检索未完成，仍显示文件名命中。',
            searchUpgrade: '当前仅搜索文件名和路径；升级并安全刷新此会话的 Agent 后可搜索正文。',
            searchCurrentBody: '搜索此范围内文件的当前正文，不检索历史差异。',
            searchPathLimit: '正文检索包含会话文件索引，以及已加载记录中的前 2,000 条文件链接。',
            searchSkippedCount: ({ count }: { count: number }) => `${count} 个文件未检索`,
            searchUnreadable: '文件不存在、无法读取、已加密或已损坏',
            searchUnsupportedFormat: '暂不支持此二进制格式或文字编码',
            searchTooLarge: '超过检索上限：文本 2 MiB、文档 16 MiB、PDF 200 页或提取文字 2 MiB',
            searchNoText: '没有可提取文字；扫描文档需要 OCR',
            searchTimeout: '正文提取超时',
            searchSensitive: '凭据文件仅搜索文件名',
            searchFileChanged: '读取时文件发生变化，请重新搜索',
            searchEmpty: '没有匹配的资源，请更换关键词或清空筛选。',
            searchEmptyPartial: '已检索文件中没有匹配；部分文件正文未能检索。',
            searchLine: ({ value }: { value: number }) => `第 ${value} 行`,
            searchPage: ({ value }: { value: number }) => `第 ${value} 页`,
            searchParagraph: ({ value }: { value: number }) => `第 ${value} 段`,
        ...
        queue: {
            ...
            steerNotNow: '这条暂时无法加入当前回合（设置不同、命令或回合已结束），仍在队列中',
            steerAttachments: '附件暂时无法引导，仍在队列中。新版 Agent 支持 PNG、JPEG、GIF、WebP 图片；其他文件请等待下一轮或选择插入。',
