import { authRevocations } from '@/lmc/auth';
...
export function startSocket(app: Fastify) {
    const io = new Server(app.server, {
        cors: {
            origin: process.env.LMC_PUBLIC_ORIGIN || 'http://127.0.0.1:4193',
    ...
    app.addHook('preClose', async () => { await new Promise<void>(resolve => io.close(() => resolve())); });
    ...
    io.use(async (socket, next) => {
        const browserOrigin = socket.handshake.headers.origin;
        if (browserOrigin && browserOrigin !== (process.env.LMC_PUBLIC_ORIGIN || 'http://127.0.0.1:4193')) return next(new Error('Origin rejected'));
        ...
        const verified = await auth.verifyToken(token).catch(() => null);
        if (!verified) {
        ...
        // Read both spellings: a client older than the rename still sends the
        // previous one, and dropping it here would silently lose its metric labels.
        socket.data.lmcClient = socket.handshake.auth.lmcClient as string
            || socket.handshake.auth.happyClient as string
            || socket.handshake.headers['x-lmc-client'] as string
    ...
    io.on("connection", (socket) => {
        const token = socket.handshake.auth.token as string;
        const revokeToken = (revoked: string) => { if (token === revoked) socket.disconnect(true); };
        const revokeHash = (hash: string) => { if (createHash('sha256').update(token).digest('hex') === hash) socket.disconnect(true); };
        const revokeUser = (user: string) => { if (socket.data.userId === user) socket.disconnect(true); };
        authRevocations.on('token', revokeToken); authRevocations.on('user', revokeUser); authRevocations.on('hash', revokeHash);
        const validity = setInterval(() => { void auth.verifyToken(token).then(value => { if (!value) socket.disconnect(true); }).catch(() => socket.disconnect(true)); }, 30_000);
        (validity as unknown as NodeJS.Timeout).unref();
        socket.once('disconnect', () => { clearInterval(validity); authRevocations.off('token', revokeToken); authRevocations.off('user', revokeUser); authRevocations.off('hash', revokeHash); });
        ...
        const lmcClient = socket.data.lmcClient as string | undefined;
