...
const {
    ...
    const sessionMessageFindMany = vi.fn(async (args: any) => {
        ...
        if (Array.isArray(args?.where?.id?.in)) {
            rows = rows.filter(message => args.where.id.in.includes(message.id));
        }
    ...
    const sessionMessageCreateMany = vi.fn(async (args: any) => {
        const createdAt = new Date(state.nowMs);
        state.nowMs += 1;
        args.data.forEach((data: any) => {
            const row: MessageRecord = {
                id: data.id || `msg-${state.nextMessageId}`,
    ...
    const txClient = {
        ...
        sessionMessage: {
            ...
            createMany: sessionMessageCreateMany
        },
        account: {
...
describe("v3SessionRoutes", () => {
    ...
    it("sends multiple messages with sequential seq numbers", async () => {
        ...
        expect(dbMock.sessionMessage.createMany).toHaveBeenCalledTimes(1);
        expect(dbMock.sessionMessage.createMany).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it("does not write or reserve sequences when every message already exists", async () => {
        ...
        expect(dbMock.sessionMessage.createMany).not.toHaveBeenCalled();
