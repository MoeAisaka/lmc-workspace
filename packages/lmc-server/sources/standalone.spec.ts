...
describe("isStandaloneEntrypoint", () => {
    it("recognizes standalone script paths on Windows and POSIX", () => {
        ...
        expect(isStandaloneEntrypoint("/repo/packages/lmc-server/sources/standalone.ts")).toBe(true);
        expect(isStandaloneEntrypoint("/repo/packages/lmc-server/dist/happy-server")).toBe(true);
    ...
    it("rejects unrelated entrypoints", () => {
        ...
        expect(isStandaloneEntrypoint("/repo/packages/lmc-server/sources/main.ts")).toBe(false);
    });
});
