 ...
 *
 * prisma/ is a build artifact here — it is copied from packages/lmc-server by
...
if (!fs.existsSync(schema)) {
  console.log('lmc-server-self-host: no prisma/schema.prisma yet, skipping generate (run build first).');
  process.exit(0);
}
