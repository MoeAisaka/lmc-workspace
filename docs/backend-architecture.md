...
This document describes the LMC backend structure as implemented in `packages/lmc-server`. It focuses on how the server is wired, how data flows through the system, and which subsystems handle which responsibilities.
...
## Process lifecycle
Entry point: `packages/lmc-server/sources/main.ts`.

```mermaid
flowchart TD
    Start([main.ts]) --> DB[Connect MySQL]
...
graph TB
    ...
    Router --> |persistent update| DB[(MySQL)]
...
graph TB
    ...
    C1 & C2 & C3 & C4 & C5 & C6 --> |opaque blobs| DB[(MySQL)]
...
- Entrypoint: `packages/lmc-server/sources/main.ts`
- API server: `packages/lmc-server/sources/app/api/api.ts`
- Socket server: `packages/lmc-server/sources/app/api/socket.ts`
- Event routing: `packages/lmc-server/sources/app/events/eventRouter.ts`
- Presence: `packages/lmc-server/sources/app/presence`
- Storage: `packages/lmc-server/sources/storage`
- Prisma schema: `packages/lmc-server/prisma/schema.prisma`
