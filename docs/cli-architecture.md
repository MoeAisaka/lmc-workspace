...
This document describes the LMC CLI (`packages/lmc-cli`) and its daemon. The CLI is both an interactive tool and a background session manager that keeps machine state in sync with the server.
...
- **Persistence/config:** `src/persistence.ts` + `src/configuration.ts` manage local state in `~/.lmc/agent`.
...
Local state lives under `~/.lmc/agent` (or `LMC_HOME_DIR`):
...
- `LMC_SERVER_URL` and `LMC_WEBAPP_URL` override defaults.
...
- CLI entry: `packages/lmc-cli/src/index.ts`
- Daemon: `packages/lmc-cli/src/daemon`
- Control server/client: `packages/lmc-cli/src/daemon/controlServer.ts`, `packages/lmc-cli/src/daemon/controlClient.ts`
- API clients: `packages/lmc-cli/src/api`
- Persistence: `packages/lmc-cli/src/persistence.ts`
- Config: `packages/lmc-cli/src/configuration.ts`
