...
**File:** `packages/lmc-cli/src/api/types.ts` (line 130)
...
**File:** `packages/lmc-app/sources/sync/storageTypes.ts` (line 119)
...
**File:** `packages/lmc-cli/src/daemon/run.ts`
...
**File:** `packages/lmc-cli/src/daemon/run.ts` (line 27-34)
...
**File:** `packages/lmc-cli/src/api/apiMachine.ts` (line 299-312)
...
The `detectCLIAvailability` function needs to be importable from apiMachine.ts — put it in a shared util like `packages/lmc-cli/src/utils/detectCLI.ts`.

### 1.6 Delete `useCLIDetection` hook from app

**File:** `packages/lmc-app/sources/hooks/useCLIDetection.ts` — **DELETE**
...
| File | Reason |
|------|--------|
| `packages/lmc-app/sources/sync/profileUtils.ts` | Built-in profile definitions & docs |
| `packages/lmc-app/sources/sync/profileSync.ts` | Profile sync service |
| `packages/lmc-app/sources/components/ProfileEditForm.tsx` | Profile editor form |
| `packages/lmc-app/sources/components/EnvironmentVariablesList.tsx` | Env var list component |
| `packages/lmc-app/sources/components/EnvironmentVariableCard.tsx` | Env var card component |
| `packages/lmc-app/sources/hooks/useEnvironmentVariables.ts` | Queries daemon env vars for profiles |
| `packages/lmc-app/sources/hooks/envVarUtils.ts` | Env var substitution utils |
| `packages/lmc-app/sources/hooks/useCLIDetection.ts` | Replaced by daemon-side detection |
| `packages/lmc-app/sources/app/(app)/settings/profiles.tsx` | Profile settings page |

### 2.2 Files to EDIT — remove profile references

**`packages/lmc-app/sources/sync/settings.ts`**
...
**`packages/lmc-app/sources/components/AgentInput.tsx`**
...
**`packages/lmc-app/sources/components/SettingsView.tsx`**
- Remove: profiles settings Item (lines 325-330)

**`packages/lmc-app/sources/sync/ops.ts`**
...
**`packages/lmc-cli/src/persistence.ts`**
...
**`packages/lmc-cli/src/daemon/run.ts`**
...
| File | Reason |
|------|--------|
| `packages/lmc-app/sources/app/(app)/new/index.tsx` | Main wizard page — will be replaced with new simpler screen |
| `packages/lmc-app/sources/app/(app)/new/pick/machine.tsx` | Machine picker sub-screen (uses SearchableListSelector with favorites) |
| `packages/lmc-app/sources/app/(app)/new/pick/path.tsx` | Path picker sub-screen (uses SearchableListSelector with favorites) |
| `packages/lmc-app/sources/app/(app)/new/pick/profile-edit.tsx` | Profile edit sub-screen |
| `packages/lmc-app/sources/components/NewSessionWizard.tsx` | Legacy wizard component |
| `packages/lmc-app/sources/utils/tempDataStore.ts` | Temp data between wizard screens |
...
**`packages/lmc-app/sources/app/(app)/_layout.tsx`**
- Remove: Stack.Screen entries for `new/index`, `new/pick/machine`, `new/pick/path`, `new/pick/profile-edit` (lines 300-327)

**`packages/lmc-app/sources/components/MainView.tsx`** — keep `router.push('/new')` (route stays the same)

**`packages/lmc-app/sources/components/SidebarView.tsx`** — keep `router.push('/new')`

**`packages/lmc-app/sources/components/HomeHeader.tsx`** — keep `router.push('/new')`

**`packages/lmc-app/sources/components/CommandPalette/CommandPaletteProvider.tsx`** — keep `router.push('/new')`

**`packages/lmc-app/sources/components/EmptySessionsTablet.tsx`** — keep `router.push('/new')`

All navigation stays at `/new` — no route changes needed.

**`packages/lmc-app/sources/sync/persistence.ts`**
...
**`packages/lmc-app/sources/app/(app)/new/index.tsx`** — replace with stub that:
