...
- ⚠️ `packages/lmc-cli` and `packages/lmc-app` do not define a `lint` script; verification used full test suites plus `yarn typecheck` in both packages.
...
- [x] Create `packages/lmc-cli/src/sessionProtocol/types.ts` with:
...
- [x] Add `sendSessionProtocolMessage(envelope)` to `ApiSessionClient` in `packages/lmc-cli/src/api/apiSession.ts`
