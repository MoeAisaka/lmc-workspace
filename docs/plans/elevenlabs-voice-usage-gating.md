...
> `deploy/handy.yaml` referenced below is an upstream Kubernetes manifest that is not
> part of this repository's published tree.
...
- Server token route: `packages/lmc-server/sources/app/api/routes/voiceRoutes.ts`
- Client token fetch: `packages/lmc-app/sources/sync/apiVoice.ts`
- Voice start decision: `packages/lmc-app/sources/realtime/RealtimeSession.ts`
- ElevenLabs client session start:
  - `packages/lmc-app/sources/realtime/RealtimeVoiceSession.tsx`
  - `packages/lmc-app/sources/realtime/RealtimeVoiceSession.web.tsx`

## Existing Secret Assumptions

The repo already assumes ElevenLabs API access exists on the server:

- `packages/lmc-server/sources/app/api/routes/voiceRoutes.ts` reads `process.env.ELEVENLABS_API_KEY`.
- `packages/lmc-server/deploy/handy.yaml` extracts `/handy-elevenlabs`.
...
Update `packages/lmc-server/sources/app/api/routes/voiceRoutes.ts` to:
...
- `packages/lmc-app/sources/realtime/types.ts`
- `packages/lmc-app/sources/realtime/RealtimeVoiceSession.tsx`
- `packages/lmc-app/sources/realtime/RealtimeVoiceSession.web.tsx`
- `packages/lmc-app/sources/realtime/RealtimeSession.ts`
- `packages/lmc-app/sources/sync/apiVoice.ts`
