...
## Context
- **Claude spawn point**: `packages/lmc-cli/src/claude/claudeLocal.ts:241` - `spawn('node', [claudeCliPath, ...args], {env, ...})`
- **Codex spawn point**: `packages/lmc-cli/src/codex/codexMcpClient.ts:107` - `StdioClientTransport({ command: 'codex', args: ['mcp-server'], env })` which internally calls `cross-spawn('codex', ['mcp-server'])`
- **Config storage**: `packages/lmc-cli/src/persistence.ts` - Settings interface + Zod schemas + atomic `updateSettings()`
- **Command dispatch**: `packages/lmc-cli/src/index.ts` - manual `if/else if` routing on `args[0]`
- **Existing command pattern**: `packages/lmc-cli/src/commands/connect.ts` - exported `handleXxxCommand(args)` functions
...
- [x] Run `yarn add @anthropic-ai/sandbox-runtime inquirer` in `packages/lmc-cli`
- [x] Run `yarn add -D @types/inquirer` in `packages/lmc-cli`
...
- [x] Create `packages/lmc-cli/src/sandbox/config.ts`
...
- [x] Create `packages/lmc-cli/src/sandbox/manager.ts`
...
- [x] Create `packages/lmc-cli/src/commands/sandbox.ts`
...
- ⚠️ Lint blocker: `packages/lmc-cli` has no `eslint.config.*` / `.eslintrc*`, so ESLint 9 cannot run in this package.
