...
Add to `Settings` in `packages/lmc-cli/src/persistence.ts:37`:
...
`packages/lmc-cli/src/configuration.ts:32` becomes:
...
And `packages/lmc-app/sources/sync/serverConfig.ts:10-14` gets one extra fallback:

```ts
export function getServerUrl(): string {
  return serverConfigStorage.getString(SERVER_KEY) ||
         (globalThis as any).__HAPPY_CONFIG__?.serverUrl ||  // NEW
         process.env.EXPO_PUBLIC_LMC_SERVER_URL ||
         DEFAULT_SERVER_URL;
}
```
...
- **happy-app (web + mobile): PostHog only**, already gated: `tracking = config.postHogKey ? new PostHog(...) : null` (`packages/lmc-app/sources/track/tracking.ts:4`). Every call site is `tracking?.capture(...)`, so absence of the key already short-circuits everything.
...
**`packages/lmc-cli/src/commands/server.ts`** (~80 LOC)
```
...
**`packages/lmc-cli/scripts/build-server-assets.mjs`** (~50 LOC, build-time)
- `pnpm --filter lmc-app build:web`
- Copy webapp dist, pglite wasm, server migrations → `packages/lmc-cli/server-assets/`

**`packages/lmc-server/sources/index.ts`** (NEW, ~40 LOC)
...
### Analytics
Only `happy-app` has PostHog (`packages/lmc-app/sources/track/tracking.ts:4`), already gated by `config.postHogKey`. CLI / agent / server have zero analytics (grepped).

### Server URL configurability (already exists)
- CLI / agent: `HAPPY_SERVER_URL` env var (default `https://api.cluster-fluster.com`).
- Web app: in-app Settings → Server screen + `EXPO_PUBLIC_LMC_SERVER_URL`.
- Server: `pnpm standalone:dev` runs with PGlite — no Docker, no Redis. Entry: `packages/lmc-server/sources/standalone.ts`.
