# Multi-process lmc-server

> The Kubernetes manifests and `deploy/integration-tests/` referenced below are upstream
> assets that are not part of this repository's published tree; the supported deployment
> path is the Compose stack in [`deploy/lmc/`](../deploy/lmc/README.md).
...
```
.
├── packages/lmc-server/sources/app/
...
│
└── packages/lmc-server/deploy/handy.yaml  k8s Deployment + Service
