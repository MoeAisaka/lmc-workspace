...
```bash
git clone https://github.com/MoeAisaka/link-my-cli.git
cd link-my-cli
pnpm install
```
...
```bash
pnpm --filter lmc-app start          # Expo dev server
pnpm --filter lmc-app ios:dev        # iOS simulator
pnpm --filter lmc-app android:dev    # Android emulator
pnpm web                                # Browser (shortcut)
pnpm --filter lmc-app typecheck      # Run after all changes
```
...
|---------|-----------|----------|----------|
| Development | `com.slopus.happy.dev` | LMC (dev) | Local development with hot reload |
| Preview | `com.slopus.happy.preview` | LMC (preview) | Beta testing & OTA updates |
| Production | `com.ex3ndr.happy` | Link my Cli | App Store release |
...
```bash
pnpm --filter lmc-app tauri:dev      # Run with hot reload
pnpm --filter lmc-app tauri:build:dev
```

### LMC CLI

```bash
pnpm --filter link-my-cli build
pnpm --filter link-my-cli test
pnpm --filter link-my-cli cli:install   # Build + link this workspace as the global `lmc` + restart daemon
```

`cli:install` replaces the `lmc` binary installed from npm with a symlink to this workspace.
It reuses `~/.lmc/agent/` (auth, sessions) — no separate dev home. To undo:

```bash
npm unlink -g link-my-cli && npm i -g link-my-cli@latest
```

To sandbox dev data, set `LMC_HOME_DIR=~/.lmc-dev` in your shell before running `lmc`.

### LMC Server

```bash
pnpm --filter lmc-server standalone:dev   # Local server (no Docker needed)
```

Runs on `localhost:3005` with embedded PGlite. To point the app at your local server:

```bash
EXPO_PUBLIC_LMC_SERVER_URL=http://localhost:3005 pnpm --filter lmc-app start
# Native only. On web, getServerUrl() returns window.location.origin, which wins over
# any EXPO_PUBLIC_* value — a web build always talks to the origin it was served from.
```
...
- **lmc-app** — React Native + Expo mobile/web client
- **lmc-cli** — Node.js CLI that wraps Claude Code and Codex
- **lmc-agent** — Remote agent control
- **lmc-server** — Backend for encrypted sync

For architecture details, check the [docs/](.) folder or ask LMC itself — it knows how the project is set up.
