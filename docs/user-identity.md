...
How a single LMC user is identified across every external service.

## Primary ID: LMC Account CUID
...
| System | ID Type | Why |
|--------|---------|-----|
| ElevenLabs | HMAC-derived | Privacy — raw LMC ID never sent to ElevenLabs |
...
To derive an ElevenLabs user ID from an LMC CUID locally:
