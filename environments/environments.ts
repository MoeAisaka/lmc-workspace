...
// ============================================================================
// Name generation (expanded from packages/lmc-app/sources/utils/generateWorktreeName.ts)
// ============================================================================
...
export async function createEnvironment(opts?: { noSwitch?: boolean }): Promise<string> {
    ...
    const standaloneTs = path.join(REPO_ROOT, "packages", "lmc-server", "sources", "standalone.ts");
    const result = spawnSync(
        "tsx",
        [standaloneTs, "migrate"],
        {
            cwd: path.join(REPO_ROOT, "packages", "lmc-server"),
...
export async function startEnvironmentServices(name: string): Promise<void> {
    ...
    const webPid = spawnService("pnpm", ["web", "--port", String(config.expoPort)], {
        cwd: path.join(REPO_ROOT, "packages", "lmc-app"),
...
export async function seedEnvironment(name: string): Promise<void> {
    ...
    const happyBin = path.join(REPO_ROOT, "packages", "lmc-cli", "bin", "happy.mjs");
...
function commandRun(service: string, serviceArgs: string[] = []) {
    ...
    switch (service) {
        ...
        case "cli": {
            ...
            const cliBin = path.join(REPO_ROOT, "packages", "lmc-cli", "bin", "happy.mjs");
            const result = spawnSync(
                "node",
                [cliBin, ...serviceArgs],
                {
...
function writeEnvCommands(envDir: string): void {
    ...
    const commands = [
        {
            name: "happy",
            entrypoint: path.join(REPO_ROOT, "packages", "lmc-cli", "bin", "happy.mjs"),
        },
        {
            name: "happy-agent",
            entrypoint: path.join(REPO_ROOT, "packages", "lmc-agent", "bin", "happy-agent.mjs"),
        },
    ];
...
async function commandUp(template: Template, opts?: { noSwitch?: boolean }) {
    ...
    if (template === "authenticated-empty") {
        ...
        const buildResult = spawnSync("pnpm", ["build"], {
            cwd: path.join(REPO_ROOT, "packages", "lmc-cli"),
            env: mergedEnv,
            stdio: "inherit",
        });
